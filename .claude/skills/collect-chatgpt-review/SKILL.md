...
If `visibilityState !== "visible"`, log a warning and proceed — Chrome MCP commands generally make the tab visible enough for our queries. If multiple collect attempts fail to detect a ready response, escalate by re-navigating to the URL (full page load forces a fresh render).
