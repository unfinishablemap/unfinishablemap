...
### 3.7 Altered-State Symmetry Remediation (Required When Pessimistic-Review or the Altered-State Symmetry Audit Flagged the Article)

When the task notes reference an altered-state symmetry flag — generated by `/pessimistic-review` (see its *Altered-State Symmetry* checklist), the `tools/curate/altered_state_symmetry.py` scanner, or an outer review surfacing the same failure mode — the refine pass installs the symmetric accommodation that the article was missing. See [[project/calibration-audit-triple]] Audit Two for the full discipline; this section specifies the action.

The failure mode the refine corrects: the article cites a *supportive cluster* of altered states (psychedelics, NDEs, terminal lucidity, contemplative cessation, mystical experience, OBE) as convergent evidence for filter / transmission framing, treating each case as an independent confirmation, while the parallel *disruptive cluster* (anaesthesia, slow-wave sleep, brain damage, persistent vegetative state, dementia) is absent or only mentioned in passing. The 2026-05-14 refine pass on `psychedelics-and-the-filter-model.md` worked out the canonical fix; the prose that landed there is the reference template for this remediation.

**The remediation has three required moves:**

1. **Name the disruptive cluster.** Add a paragraph that explicitly names which disruptive-cluster items the filter framing must also accommodate. The task notes from the audit will list which items are missing — install them by name, not behind a generic phrase like "other altered states." If the article already mentions an item in passing, expand to engage it as evidence the framing owes work on, not as a footnote.
2. **State how the framing accommodates them.** The filter framing's standard accommodation for the disruptive cluster: anaesthesia narrows the interface (propofol and xenon target phenomenal-presence specifically; ketamine severs access channels while sparing phenomenal content); slow-wave sleep is interface-bandwidth-driven-to-near-zero; brain damage and dementia constrain the filter structure rather than abolishing the consciousness the structure filters. Cite [[topics/anaesthesia-and-the-consciousness-interface]], [[topics/dream-consciousness]], and the relevant cases from [[concepts/altered-states-of-consciousness]] for the corpus-level accommodation work that the audited article should inherit. Do *not* invent new accommodation moves — use the moves the Map already makes.
3. **Mark the accommodation as parallel — not stronger than — the production framing's symmetric work.** This is the load-bearing move. The filter framing's accommodation of the disruptive cluster is *structurally identical* to the production framing's accommodation of the supportive cluster. The honest framing of the dialectical situation is: filter theory accommodates the full altered-state set; production theory accommodates the same set by parallel moves; the supportive cluster cannot be cited as multiple independent confirmations of filter framing without granting the symmetric concession that the disruptive cluster is a parallel test of production framing. The reference phrasings from the canonical fix: *"the same move is available to production theorists"*, *"structurally identical accommodations"*, *"the cluster carries the evidential weight of one pattern, not five"*, *"cannot honestly be cited as multiple independent confirmations"*. Use these or natural-language equivalents.

**What the refine should not do.** Do not delete the article's filter-framing argument or weaken its overall claim. The discipline is *not* a request to retract — it is a request to make the dialectical situation honest. The article can still endorse filter theory; what it cannot do is claim the supportive cluster constitutes *N* independent confirmations while ignoring the disruptive cluster's parallel burden. Preserve the original conclusion; install the symmetric accommodation so the conclusion is reached honestly.

**Cross-reference discipline.** The refine should add wikilinks to [[topics/anaesthesia-and-the-consciousness-interface]] (or whichever disruptive-cluster article is most relevant) and to [[concepts/altered-states-of-consciousness]] where the corpus-level convergence treatment lives. If the article is itself a concept-tier piece, the same cross-references apply but the prose should pitch one tier more abstract.

**Verification.** After the refine, the article should clear `tools/curate/altered_state_symmetry.py` — `evaluate_symmetry(path)` returns `None`. If the function still returns a `SymmetryFlag` after the refine, either the disruptive cluster is still missing, the symmetry-acknowledgment language did not land in the body (perhaps it leaked into a footnote, or the chosen phrasing is not in the marker set), or the supportive-cluster items have grown such that the disruptive engagement is no longer proportionate. Re-read the audit's flag notes and the marker patterns in `tools/curate/altered_state_symmetry.SYMMETRY_ACKNOWLEDGMENT_PATTERNS`; install the language the audit can recognise.

The corresponding pessimistic-review check is the upstream that should catch the failure mode at review time; this remediation is the downstream that closes it.
