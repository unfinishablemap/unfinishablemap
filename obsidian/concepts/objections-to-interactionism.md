...
For physical causation, spatial relations provide the answer. The billiard ball strikes *this* ball because they occupy adjacent positions. But if minds are non-spatial (as traditional substance dualism holds), no spatial relation can ground the pairing. Kim argues: "Causal relations between physical objects or events appear to depend crucially on their spatiotemporal relations to each other." If causation requires spatial relations, and minds lack spatial location, minds cannot enter into causal relations at all.
...
- In the **expanding universe**, cosmological red-shift means photon energy is not globally conserved
...
the project's primary response: at quantum indeterminacies, consciousness selects among possible outcomes without adding energy. The outcomes all have the same energy; consciousness just determines *which* one actualizes.
...
Parsimony applies only when theories have equal explanatory power. Physicalism notoriously struggles with consciousness. The [[hard-problem-of-consciousness|hard problem]] shows that physical descriptions—however complete—don't entail phenomenal experience. The [[explanatory-gap]] between physical structure and subjective quality has resisted closure despite decades of effort.

A simpler theory that *doesn't explain the data* isn't preferable. If materialism cannot account for consciousness, its apparent parsimony is irrelevant.

### Response 2: Type vs. Token Parsimony

Dualism adds one ontological *type* (mind), not millions of *tokens*. Adding one well-motivated category isn't extravagant parsimony violation. Physics itself posits multiple fundamental types—fermions, bosons, four fundamental forces—without parsimony complaint. Why should consciousness be different?
...
This is the project's [[tenets#^occams-limits|fifth tenet]]. Parsimony is a methodological heuristic, not a metaphysical truth. It has repeatedly pointed away from truth in the history of science:
...
- Quantum mechanics replaced deterministic simplicity with probability amplitudes and superposition
...
the project can use this argument *offensively* against epiphenomenalism: if consciousness causes nothing, evolution can't explain the observed correlations. The precise tracking of phenomenal states with fitness-relevant behaviors would be an inexplicable coincidence.
...
4. **Conclusion**: M either is identical to P (reductionism) or causes nothing (epiphenomenalism)
...
the project's primary response: the exclusion argument assumes that physical properties provide *sufficient* causes. But at quantum indeterminacies, P is *not* sufficient for E—physics determines probabilities but not outcomes.
...
2. **MWI confirmation**: If the [[many-worlds|Many-Worlds interpretation]] were empirically confirmed—if branching could be detected and the probability interpretation derived—then all outcomes occur and "selection" is illusory. the project's No Many Worlds tenet would fail.
...
The objections to interactionism motivate rather than undermine the project's framework.
...
The [[tenets#^occams-limits|Occam's Razor Has Limits]] tenet addresses parsimony: simplicity is unreliable when knowledge is incomplete. Materialism's apparent simplicity reflects inability to explain, not success in explaining.

The [[tenets#^bidirectional-interaction|Bidirectional Interaction]] tenet affirms what the evolutionary argument requires: consciousness causes physical effects. The precision of valence-fitness correlation is explicable because phenomenal states genuinely influence behavior.
...
- [[arguments-for-dualism]] — Positive arguments for the project's position
...
- [[many-worlds]] — The interpretation whose rejection grounds the project's view
