...
The constraint binds the *marginal* distribution over outcomes and nothing else. [[born-preserving-causal-efficacy]] states the relation exactly: on the averaging identity q(O | X) = Σ_C P(O | C, X) · P(C | X), Born-preservation constrains the left-hand marginal and says nothing about whether the conditionals on the right depart from it. What preservation secures is invisibility to *unconditioned* aggregate frequency tests, leaving information flow across the channel entirely open.
