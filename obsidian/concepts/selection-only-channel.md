...
The per-event log₂(N) ceiling survives untouched as the binding bound on a single selection. What changes is where the channel's information would show — in *conditional* structure, so the tests that bear on it are conditional residual-structure tests rather than generic Born-frequency tests. That is what still separates this channel from a generic Shannon channel of the same per-event capacity: a generic channel leaves its signature in the output's own statistics, and this one cannot, because those statistics are pinned to the physical-side prior whatever the mind-side input does. [[born-preserving-causal-efficacy]] develops the resulting trilemma: conditionals that differ, conditionals that never differ, or conditionals that differ yet cancel under a balancing law the framework would then owe.
...
- [[conservation-laws-and-mental-causation]]
- [[born-rule-and-the-consciousness-interface]]
- [[born-preserving-causal-efficacy]] — Why marginal Born-preservation leaves the conditionals free
- [[agency-budget]] — What survives exact Born-preservation read per publicly conditionable context: a coupling-theoretic budget, its min(H(conscious source), H(Born distribution)) ceiling, and — within deterministic hidden-variables theories — Valentini's (2002) signal-locality theorem as the price of the weak reading
- [[interface-specification-programme]]
- [[possibility-probability-slippage]]
