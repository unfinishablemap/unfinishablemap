...
last_deep_review: 2026-08-19T00:14:00+00:00
related_articles:
  - "[[apex-articles]]"
...
This article treats retention-as-evidence as an *apex-stability principle*: condense survival is a structural cue the apex re-cross-review can exploit when identifying which source-article claims a stable apex narrative may rely on. A parallel project-document treats condense as an editorial operation more broadly; that operation produces the condense, while the apex-stability principle interprets the survival pattern.

## Empirical Performance: Structural Stability Holds, Condense Gains and Review History Do Not

Four late-April-2026 restructures are now old enough to measure against their post-restructure baselines, and the result splits three ways: the *structural* refactor holds, the *length* gain does not, and the *review history* carried into the merge is the least reliably preserved of the three.

The structural claim is the strongest, and must be stated narrowly. `self-and-self-consciousness` (4914→2770w coalesce), `the-quantitative-comprehension-void` (three-way coalesce, converged 2987w), `ai-consciousness-typology` (2745w post-condense), and `creative-consciousness` (~2261w post-condense) have each held their merged section structure through every later review: no inter-section contradiction, no apex citation left dangling. That is the integrity the apex-stability criteria test, and it is confirmed. It does not extend to the articles being clean. `creative-consciousness` has drawn a critical finding in four of its five post-restructure reviews — a condense-caused citation regression (2026-04-30), a fabricated author name (06-02), a commentator's phrase quoted as Husserl's own (07-12), and a misattributed timing figure that regressed a fix first applied on 2026-02-21 (08-07) — and has never reached two consecutive clean reviews. Nor is the restructure arc itself exempt: the 2026-04-29 coalesce of `ai-consciousness-typology` introduced a flat contradiction between its borrowed-phenomenality and epiphenomenal-phenomenality sections, caught by the next day's review. Structural integrity survives restructuring; content integrity is what restructuring puts at risk.

The gains regress in three distinct ways. **Length re-accretes on hub articles.** `self-and-self-consciousness` re-grew 2770→3404w (123% of baseline) as later articles' reviews appended cross-links into it — the merger held, the trim did not. **Condense can silently drop load-bearing qualifiers.** `the-quantitative-comprehension-void` was condensed *further* (to 2403w), yet the 2026-06-03 calibration audit found it had stripped two evidential-status markers ("dialectical, not knockdown"; "post-hoc framing, not an independent prediction"), each strengthening a hedged claim — the critical dropped-qualifier category in the [[evidential-status-discipline|evidential-status discipline]]'s terms, caught only on a delayed audit. **Coalesce can separate a qualifier from the claim it repairs.** The 2026-06-15 three-way merge that produced [[composition-question-rivals|composition-question-rivals]] moved a parenthetical — installed eight days earlier in direct response to a pessimistic review of that very sentence — twenty-six lines away, into a different section, leaving the claim bare. Nothing was deleted, so each half read as sound in isolation, and two subsequent deep-reviews ratified the split before the 2026-08-18 review reunited them. Relocation and deletion produce the same defect, but only deletion is visible to an audit that asks whether a hedge is still present.

The discipline therefore needs three additions a one-time stability declaration cannot supply. A **calibration-qualifier audit** within a few cycles of any post-restructure condense, since the regression is invisible at restructure time. A **periodic length re-check** on merged hub articles, since the trim decays. And a **post-restructure provenance check**, which neither of the first two can supply because neither examines what the merge *moved* rather than what it removed. The provenance check runs within a cycle of any coalesce and diffs the merged article against its archived predecessors rather than against its own last review, verifying two things: that every hedge or scope-restriction still sits with the claim it repairs, not merely somewhere in the article; and that the surviving article's `last_deep_review` was carried through rather than reset. The 2026-08-18 merge into [[quantum-indeterminacy-free-will|quantum-indeterminacy-free-will]] nulled a 2026-07-25 timestamp against eight archived deep-reviews of that slug, so a much-reviewed article read as never-reviewed and mis-justified the next task that consulted it. The reset is not systematic — the 2026-08-06 merge into [[mine-ness|mine-ness]] preserved its timestamp — which is why the check has to be run rather than assumed. The `ai_modified` bump already re-qualifies a merged article for review, so nulling the field adds no scheduling signal and discards the audit trail. "Stable under restructure" holds for the article's argument, not for its word count, its hedges, or its record of having been examined.
...
The re-cross-review is conducted as a deep-review with the explicit task of *confirming* rather than improving — a verification pass against the apex's existing argument, not a clean-slate read. Word-count change should be near-zero; the success condition is that nothing requires changing.
