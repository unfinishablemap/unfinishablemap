...
description: "Vision divides between brain-side computation and mind-side phenomenal contribution. Blindsight, the two-streams hypothesis, and perceptual organisation mark where the division falls."
created: 2026-03-17
modified: 2026-03-17
human_modified:
...
last_curated:
last_deep_review: 2026-09-19T09:26:40+00:00
---
...
**In-practice only conditional on instrumentation the framework does not have.** (3) A computational model deriving the three flexible-deployment capabilities — durable maintenance, novel combination, spontaneous intentional action — from ventral-stream architecture alone would remove the first of the two grounds §"The Epiphenomenalist Pressure" rests on. The obstacle is that nothing currently distinguishes a model that produces flexible deployment from a model that produces *conscious* flexible deployment; [[the-interface-problem|the interface problem]] catalogues why that distinction has no available empirical criterion. (4) A dorsal-stream finding that visuomotor control is accompanied by phenomenal experience the subject cannot report would undercut the two-streams reading — but an unreportable experience is exactly what the framework has no instrument for. Both become in-practice items if that instrumentation arrives.

**In-principle (inaccessible by the framework's own commitments).** (5) Direct verification that a system implementing ventral-stream computation lacks phenomenal vision requires the joint phenomenal-physical instrumentation the [[standing-agnostic-challenge|standing agnostic challenge]] denies is available. (6) A demonstrated identity between a neural binding mechanism and phenomenal unification would settle the question against the article — but the [[explanatory-gap|explanatory gap]] on which the division rests entails that no neural specification could by itself establish the identity, so the article cannot coherently offer this as a test it might fail.

Honest framing: only (1) and (2) are genuinely in-practice, and both are boundary-placement tests rather than tests of whether there is a division at all. Items (3) and (4) wait on instrumentation the framework denies it has. Items (5) and (6) are inaccessible by the framework's own commitments, not by a gap future technology will close. The net falsifiability is modest — narrower than a six-item list invites the reader to assume — and [[evidential-status-discipline|the evidential-status discipline]] requires the article to say so rather than leave it for the reader to work out.

## Relation to Site Perspective

The Map's tenets constrain how the capability division in vision should be understood. Tenet 1 ([[tenets#^dualism|Dualism]]) establishes that the division is real and not merely epistemic — consciousness is genuinely irreducible to neural computation, so the boundary between brain-side and mind-side visual capabilities marks a genuine ontological distinction. Tenet 3 ([[tenets#^bidirectional-interaction|Bidirectional Interaction]]) ensures the mind-side is not empty — consciousness causally contributes to visual processing rather than merely accompanying it. Tenet 2 ([[tenets#^minimal-quantum-interaction|Minimal Quantum Interaction]]) explains why the boundary resists precise specification — the interface operates at a level below the resolution of current neuroscience.
...
Five further modality treatments have followed, each restating the division while removing a different convenience of the visual case. [[tactile-consciousness-and-the-interface|Touch]] removes passivity: active haptics makes the perceiver's own movement constitutive of the content, so the brain side has to include a sensorimotor loop rather than a feedforward pipeline. [[chemosensory-consciousness-and-the-interface|Smell and taste]] remove the stimulus-to-quality mapping altogether — mirror-image molecules smell categorically different — leaving no reflectance-like property for the brain side to be reading off. [[thermal-consciousness-and-the-interface|Thermoception]] removes neutrality: thermal quality arrives already valued, and the value is indexed to body state rather than to the stimulus. [[vestibular-consciousness-and-the-interface|The vestibular sense]] removes the experienced object, contributing the spatial frame of experience while barely appearing within it. [[interoceptive-consciousness-and-the-interface|Interoception]] inverts the pattern on both sides at once, converging on a shared allostatic substrate where exteroception has none, and delivering owned bodily feeling where exteroception delivers detached perceptual objects. That the division survives each removal is the strongest available evidence that it is not an artefact of vision — though [[cross-modal-capability-division|the cross-modal synthesis]] collecting these cases reads the regularity as exteroceptive rather than universal, on interoception's showing.
