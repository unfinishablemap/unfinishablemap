...
The honest Map statement is therefore a conditional: *an agent coupling to quantum outcomes in a regime that satisfies purification and local tomography — as physics does everywhere it has been tested — is Born-constrained on pain of signalling.* The constraint on a non-physical interface is exactly as strong as the applicability of those two structural axioms to the interface, and no stronger. This is an upgrade, not a retreat: it names the precise open question ([[brain-internal-born-rule-testing]]) instead of hiding it inside a slogan.
