...
the project's position: quantum indeterminacy provides the *opening* for mental causation; consciousness provides the *direction*. Neither alone suffices. Indeterminacy without direction is mere randomness; direction without indeterminacy reduces to determinism. Together, they ground genuine freedom.
...
Robert Kane, Mark Balaguer, and Laura Ekstrom defend *event-causal* libertarianism: mental events (beliefs, desires, intentions) cause actions, but through nondeterministic processes. Quantum indeterminacy provides the openness; mental events provide causation.

Kane locates indeterminacy in the *effort of will* itself. In self-forming willings (SFWs)—genuinely conflicted decisions—the agent's effort is indeterminate, analogous to position/momentum indeterminacy in quantum mechanics. The agent's trying counts as doing even when the effort's outcome isn't fixed.

His 2024 *Complex Tapestry of Free Will* explicitly incorporates agent-causal elements alongside the event-causal framework, arguing that free action requires agents "exercising teleological guidance control over some of their processes." This represents convergence between event-causal and agent-causal positions.

### Agent-Causal Libertarianism

the project's framework is [[agent-causation|agent-causal]]. Roderick Chisholm distinguished *transeunt* causation (event→event) from *immanent* causation (agent→event). E.J. Lowe argued that all causation is fundamentally substance causation—agents exercising powers—with event causation being derivative shorthand.
...
Mele sharpened this into two distinct problems:

**Present luck**: At the moment of decision, nothing about the agent explains why one option was chosen. Two worlds identical up to that moment diverge arbitrarily.
...
the project's response distinguishes *lucky indeterminism* from *consciousness-selected indeterminism*:
...
Critics press: but what explains why consciousness selected A rather than B? The [[agent-causation|agent-causal]] response: the agent's exercise of causal power is itself the explanation. Just as physics doesn't explain why the universe exists (taking existence as primitive), agent causation takes the agent's power to select as primitive.
...
[[retrocausality|Retrocausal]] interpretations of quantum mechanics offer resolution. The fundamental equations are time-symmetric—they work equally well run forward or backward. Cramer's transactional interpretation makes this explicit: quantum events involve "offer waves" propagating forward and "confirmation waves" propagating backward, with the "transaction" being atemporal.
...
The main philosophical objection to mental causation is [[causal-closure|causal closure]]: every physical event has a sufficient physical cause. If this is true, mental causes are excluded—either redundant (if identical to physical causes) or epiphenomenal (if different).

Quantum indeterminacy provides the response. At quantum measurement events, physics specifies necessary but not sufficient causes. The particle must be measured (physics determines this), but which outcome occurs is genuinely undetermined. [[causal-closure|Causal closure fails precisely where consciousness acts]].
...
the project's [[tenets#^minimal-quantum-interaction|Minimal Quantum Interaction]] tenet formalizes this: consciousness biases otherwise indeterminate quantum outcomes without injecting energy or violating conservation laws. The smallest possible non-physical influence on physical outcomes.
...
the project's [[tenets#^no-many-worlds|No Many Worlds]] tenet rejects this for several reasons:
...
the project's response: illusionism is self-undermining. If the appearance of reasoning is wholly constructed by neural processes unconnected to any real reasoning, then the decision to adopt illusionism was itself constructed—not genuinely reasoned to. The illusionist cannot coherently claim to have *reasoned* to their position without presupposing the phenomenal engagement they deny.
...
**Many-worlds empirical confirmation**: If the [[many-worlds|many-worlds interpretation]] were empirically confirmed, there would be no collapse for consciousness to influence—all outcomes occur in branching universes. the project's [[tenets#^no-many-worlds|No Many Worlds]] rejection is foundational to this picture.
...
Quantum indeterminacy alone doesn't give free will. Random outcomes are no freer than determined ones. But indeterminacy combined with consciousness as selector provides what libertarian free will requires:

1. **Alternative possibilities**: Multiple outcomes were genuinely possible until selection
2. **Sourcehood**: Consciousness is the ultimate source—it selects among what physics leaves undetermined
3. **Reasons-responsiveness**: Selection tracks what matters to the agent
4. **Phenomenological authentication**: The felt effort of choosing corresponds to genuine causal engagement

The brain prepares superposed neural patterns. Physics determines that measurement will collapse the superposition but doesn't determine which outcome. Consciousness selects—not randomly, but based on reasons, with felt effort that marks genuine engagement. The agent is the source.

This isn't proof—it's coherent integration of quantum mechanics, consciousness, and agency. The framework takes both the hard problem (consciousness is irreducible) and quantum mechanics (indeterminacy is genuine) seriously, finding in their intersection the space for libertarian free will.

## Relation to Site Perspective

This article systematizes the relationship between quantum theory and the project's core tenets:
...
- [[free-will]] — The broader free will question and this project's position
- [[luck-objection]] — Detailed treatment of the central challenge to libertarian free will
- [[agent-causation]] — Agent vs event causation and why the project's framework is agent-causal
