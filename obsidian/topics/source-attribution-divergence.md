...
This is the methodological knot the case forces. If introspective reports of source-content are unreliable, source-attribution divergence is exactly the case where one would expect the unreliability to surface as cohort divergence. The within-species spread then becomes evidence for the unreliability rather than evidence for phenomenal divergence. If introspective reports of source-content are reliable enough to register cohort differences, then the divergence is genuine phenomenal divergence and runs the standard wedge. The case does not by itself decide which of these readings is correct; it inherits the methodological dependency the entire within-species cluster operates under, and it inherits it more sharply than its cluster siblings do.

The Map's verdict is that the case carries cumulative weight in the cluster regardless of which reading is correct. Under the option-1 reading, source-attribution divergence may run as direct phenomenal divergence and the wedge against functionalism is then available. Under the option-3 reading, the divergence still suggests that introspective reports about source are at best partial guides to underlying source-attribution machinery—a finding the standard functionalist identification of phenomenology with functional role might be expected to absorb. Either way, the simple identification of phenomenal source-content with functional source-tagging appears to fail: option 1 falsifies it from the phenomenal side, option 3 falsifies it from the introspective-access side. The function-phenomenology gap seems to survive both readings; only its location shifts.
