...
- **Authorship-of-action divergence.** The choice-blindness paradigm (Johansson et al. 2005) shows that healthy adults often cannot detect when their own (manipulated) choices are reattributed to them. Detection rate varies across the population.
...
The Map's verdict is that the case carries cumulative weight in the cluster regardless of which reading is correct. Under the option-1 reading, source-attribution divergence is direct phenomenal divergence and the wedge against functionalism runs. Under the option-3 reading, the divergence still establishes that introspective reports about source are at best partial guides to underlying source-attribution machinery—which is itself a finding the standard functionalist identification of phenomenology with functional role has to absorb. Either way, the simple identification of phenomenal source-content with functional source-tagging fails: option 1 falsifies it from the phenomenal side, option 3 falsifies it from the introspective-access side. The function-phenomenology gap survives both readings; only its location shifts.

The cluster-level point is the same as at aphantasia and synaesthesia: source-attribution divergence alone is locally absorbable—a sufficiently fine-grained functionalism can individuate functional source-tagging at the level the divergence requires. The joint package of aphantasia *and* synaesthesia *and* source-attribution divergence puts strain on the absorption strategy, because the absorption has to deliver fine-grained individuation across three architecturally distinct dimensions of within-species variation while preserving task-level functional equivalence.
