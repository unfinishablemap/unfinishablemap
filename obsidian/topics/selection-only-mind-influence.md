...
That is the whole of the rate-level arithmetic the strict reading currently supports. Born-rule preservation does not add a second, tighter formula, because it does not bear on throughput at all. What it binds is the *marginal* distribution over outcomes. [[born-preserving-causal-efficacy]] states the relation exactly: on the averaging identity q(O | X) = Σ_C P(O | C, X) · P(C | X), Born-preservation constrains the left-hand marginal and says nothing about whether the conditionals on the right depart from it. What preservation secures is invisibility to *unconditioned* aggregate frequency tests, leaving information flow across the channel entirely open.
