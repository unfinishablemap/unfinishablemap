...
These are not a target list: each is a serious framework whose internal coherence accommodates the Born rule's interpretation-invariance, and discriminating among them is the work the rest of the article specifies.
...
The honest state is therefore *interface-compatible* and not yet *interface-discriminating* — and it does not reach *interface-suggestive*, because every component that might have qualified is matched by a named rival's equal move. The one candidate above bare compatibility, the SIC how-sketch below, is suggestive only if granted. The underivability pattern's job is to give the Map a coherent framework-internal home for the Born rule, not to elevate the reading above its rivals.
...
**[[envariance|Zurek's envariance]]** (2005) appeals to symmetries of entangled states. Barnum (2003) showed a natural generalisation of its auxiliary assumption yields the Born rule itself; Schlosshauer-Fine (2005) diagnose an assumed state-probability link; Mertens & van Wezel (2023) showed envariance does not necessitate it for all measurements.

**QBism** (DeBrota et al. 2021) makes the observer central by design, so an agent-centred theory predicting the failure of observer-independent derivation is doing what it should. The [[pragmatist-quantum-foundations-and-the-agent|pragmatist family]] reading treats this normativity as the structural surface of a dualist interface QBism declines to posit; the disagreement is over what the agent *is*, not over whether the Born rule presupposes one — both agree it does. QBism withholds the non-physical substrate on grounds of minimal ontology, the Map posits it on grounds of cross-corpus tenet-coherence, claiming it independently explains the SIC-geometric (d+1)/−1/d form QBism leaves as brute coherence. The how-sketch: that deformation measures the gap between an unperformed reference SIC and the single actualised outcome, and the interface reading already posits a unified experiencer actualising exactly one possibility among d ([[composition-and-consciousness|phenomenal non-compositionality]]) — so the coefficients' dimension-d non-classicality is the structural trace of d-dimensional indeterminacy meeting a one-outcome locus. That explanation-by-substrate is not an in-framework refutation; the disagreement is framework-boundary, not resolvable from Born-rule data alone.
...
Every framework postulates the Born rule, assumes it in disguised form, or relocates it; none generates its *existence* from non-probabilistic, observer-independent premises — and the Map's own interface reading earns its last row by importing assumptions of the same kind, not by escaping the audit.
...
Two features may resist purely mathematical-realist redescription, though the realist could dispute both. The deformation appears *irreducibly counterfactual* — the unperformed reference SIC is the informational ground against which actual outcomes are specified, what the interface reading takes to be the structural signature of a gap being crossed — and its (d+1)/−1/d form falls out of SIC geometry, which the realist explains only by deferral, while the interface reading offers the independent motivation sketched in the QBism paragraph above. A Hilbert-space primitivist can accept the theorem and refuse the meaning, making the Map's absorption heuristic rather than derivational.
...
If the Born rule's *existence* could be derived from physics alone, the conceptual space for consciousness at the measurement interface would narrow. That it cannot, despite a century of effort, leaves the [[quantum-probability-consciousness|interface reading]] open: on it, the Born rule describes how consciousness actualises one possibility among many — [[conservation-laws-and-mental-causation|without injecting energy]]; [[consciousness-physics-interface-formalism]] makes Born preservation the first of five non-negotiable coupling constraints, and the [[delegation-meets-quantum-selection|delegation-selection bridge]] identifies the Born distribution with Saad's "default causal profile" in [[delegatory-causation|causal delegation]]. The [[trumping-preemption|trumping reading]] locates the same openness at the authority layer instead, and each rival builds its own framework-internal home.

Being simultaneously *form-fixed* and *existence-underivable* is a duality the QBist normative reading and the objective-collapse calibration accommodate too; the interface reading offers a *natural* home, not a unique one. Tonetto locates the meeting point: quantum mechanics provides "statistical closure with outcome-level openness" — structurally complete yet ontologically partial, and that partiality is the Born rule gap ([[completeness-in-physics-under-dualism|completeness analysis]]).
