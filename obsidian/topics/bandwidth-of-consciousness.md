...
The [[resolution-void|resolution void]] and bandwidth constraint are not merely co-located at the interface — they are tightly coupled. The compression ratio determines the selection vocabulary, and the selection vocabulary determines the bandwidth requirement.
...
The converse prediction also holds: degraded interface operation should produce impoverished but epistemically revealing experience. The [[perceptual-failure-and-the-interface|blur paradox]] confirms this. Altered states involving loosened interface constraints—[[dream-consciousness|dreams]], psychedelics, meditative absorption—should involve experiences reported as *richer* than ordinary waking consciousness. Psychedelic research finds increased neural entropy correlating with expanded subjective richness (Carhart-Harris et al., 2014). Production models must explain why disrupted neural processing correlates with enriched experience; filter models expect it.
...
This matches the phenomenology of deliberate action: choosing among presented options, not computing from raw data. The [[libet-experiments|Libet readiness potential experiments]] captured this architecture: neural preparation precedes conscious awareness because preparation *must* precede selection.

The contrast with unconscious motor bandwidth reinforces the [[consciousness-selecting-neural-patterns|selection model]]. A pianist's fingers execute complex sequences at rates far exceeding conscious throughput—because those sequences were selected and delegated to automatic execution. Unconscious systems handle execution at full neural bandwidth; consciousness handles selection at interface bandwidth.

### The Global Workspace Objection

[[global-workspace-theory|Global Workspace Theory]] (Baars, 1988; Dehaene, 2014) provides the physicalist counterargument most directly targeting the asymmetry. On the GWT account, consciousness is a serial broadcast that integrates information from parallel processors. The serial bottleneck produces the ~10 bits/s ceiling; the parallel sensory channels produce the high inbound bandwidth.

The Map acknowledges GWT's explanatory power for the *functional* structure. But GWT does not address the question that motivates dualism: why is there *something it is like* to occupy the workspace? GWT explains *which* information becomes conscious and *why* processing is serial, but treats phenomenal experience as an unexplained accompaniment. The asymmetry is compatible with both GWT's architecture and the interface model. What distinguishes them is whether the architecture explains consciousness or merely describes the structure through which consciousness operates.

## The Bandwidth Shapes the Interface

### Policy-Level, Not Neuron-Level

At 10 bits per second, consciousness cannot select individual neural firing events. It selects *patterns*—action policies, attentional targets, cognitive strategies. "Reach for the cup" rather than "fire motor neuron N₄₅₆₇₈." This coarse-grained selection mirrors [[adaptive-computational-depth|adaptive computational depth]]—detail resolves where consciousness attends, not uniformly.

This matches the [[tenets#^minimal-quantum-interaction|Minimal Quantum Interaction]] tenet. Consciousness *cannot* interact at finer grain because the channel is too narrow. The minimality follows from the bandwidth constraint.

### ~3–4 Selections Per Second

Consciousness makes approximately 3–4 selections per second. Musslick et al. (2016) estimated cognitive control capacity at ~3–4 bits per second—directly measuring deliberate control rather than automated sequences. The ~280–300ms timing window for [[motor-selection|motor commitment]] and [[attention-as-interface|willed attention deployment]] aligns: conscious selection at ~3 Hz takes ~300ms per selection, precisely where neural signatures distinguish willed from automatic processing.
...
The [[baseline-cognition|baseline cognition hypothesis]] attributes sophisticated cognition—tool use, social learning, spatial navigation, pattern recognition—to unconscious systems operating at full neural bandwidth. What requires conscious bandwidth is *manipulation* rather than maintenance: logical reasoning, counterfactual thinking, recursive language. [[working-memory|Working memory]] research confirms this distinction.
...
The asymmetry poses a specific challenge to [[epiphenomenalism]]—the view that consciousness accompanies brain activity without causally influencing it.

If consciousness were epiphenomenal, the outbound "channel" would carry zero bits per second. There would be no consistent throughput ceiling, because epiphenomena have no throughput. The existence of a measurable, task-independent bandwidth—10 bits per second, not zero, not random—indicates consciousness is performing work.

Mandik (2010) identified a further asymmetry: sensory phenomenology is informationally rich while control phenomenology is sparse. Introspecting motor control introduces a "doubling of tasks"—attending to controlling interferes with controlling. This is intelligible if conscious action operates through a narrow channel that cannot simultaneously carry action-selections and self-monitoring. An epiphenomenal consciousness would face no such capacity constraints.
...
Schultze-Kraft's 2016 research identified a "point of no return" at ~200ms before movement execution—before which conscious intervention can veto prepared actions. This dissolves rather than creates the [[libet-experiments|Libet problem]]: if consciousness's role is selection among prepared options rather than initiation from nothing, neural preparation *should* precede selection.
...
1. **Conscious processing rate proves much higher than ~10 bits/second.** If consciousness routinely handles hundreds or thousands of bits per second, the bottleneck interpretation weakens. Decades of measurement across diverse domains converge on the same order of magnitude.
...
**[[tenets#^bidirectional-interaction|Bidirectional Interaction]]**: The asymmetry defines the two directions' architecture. Brain-to-consciousness runs at ~10⁹ bits per second, delivering rich experience. Consciousness-to-brain runs at ~10 bits per second, selecting among prepared options. Both directions are causally real; they differ in bandwidth by eight orders of magnitude. This is bidirectional interaction with an asymmetric channel—one narrow enough to operate within the gaps that [[causal-closure|causal closure]] leaves open.

**[[tenets#^no-many-worlds|No Many Worlds]]**: The selection model implicit in the outbound channel presupposes genuine selection—one outcome becoming actual at the expense of alternatives. If all outcomes actualise (as many-worlds holds), "selection" is illusory—every option is realised in some branch. On the Map's view, selection is genuine: consciousness chooses one possibility and the alternatives are genuinely discarded. The bandwidth asymmetry is most naturally read as describing a system where selection has real consequences.
...
- [[ethics-of-cognitive-enhancement-under-dualism]] — Ethical implications of modifying the interface's bandwidth
