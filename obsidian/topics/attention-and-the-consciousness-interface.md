...
Three signatures distinguish willed from automatic processing in both domains: frontal theta oscillations (present for willed, absent for instructed; Rajan et al. 2019), bidirectional theta-band coherence between frontal and parietal regions, and matching commitment timing (~300ms for willed attention, ~280ms before movement; Thura & Cisek 2014). If attention and motor selection were independent systems, this convergence would be coincidental.

Traditional philosophy of mind treats perception and action as separate puzzles. Because attention is motor planning, selecting what to perceive is already selecting what to do—one interface, not two.
...
The [[trilemma-of-selection|trilemma of selection]] is exhaustive: determinism, randomness, or mental causation—only the third preserves authorship. The Map proposes consciousness biases which option wins at threshold crossing, satisfying [[tenets#^minimal-quantum-interaction|Minimal Quantum Interaction]].

Cai et al. (2024, *Nature*) reveal a striking dissociation: dopamine-deficient mice retain motor capacity but lose goal-directed initiative—they "can move" but "won't move." Two layers emerge: cortical option generation (preserved without dopamine) and [[dopamine-and-the-unified-interface|dopamine]]-modulated salience and threshold. Berridge's (2007) "wanting vs. liking" distinction reinforces this—dopamine mediates incentive salience, not hedonic pleasure. Cai et al. test only these two layers; their findings are equally consistent with Berridge incentive-salience without an added third layer. The Map's interface hypothesis proposes a third layer the paper does not test — consciousness biases which salient option crosses threshold once dopamine has weighted the alternatives — and this proposal stands or falls with the broader interface argument, not with Cai et al. Parkinson's patients move when externally prompted but struggle with self-initiated movement—consistent with degradation of the salience-and-threshold layer Cai et al. confirm, and consistent with (but not by itself evidence for) interface hardware degraded while the consciousness that uses it is unchanged.
...
The 2025 COGITATE adversarial collaboration (Melloni et al., *Nature*) found neither Global Workspace Theory nor Integrated Information Theory was vindicated, with conscious content tracking posterior cortex rather than prefrontal regions. The Map's [[attention-as-interface]] hypothesis proposes attention is how consciousness *engages* with matter, not what consciousness *is*. The interface model treats dissociation as expected—a driver and their steering wheel are separable.
...
Conscious throughput, measured behaviourally, is roughly 10 bits per second (Zheng & Meister, 2024)—orders of magnitude below the rate at which sensory transduction delivers information to the brain. The asymmetry motivates [[consciousness-bandwidth-architecture|the interface-friction model]]: 10 bits/sec suffices for 3-4 selections per second among small alternative sets—the competition-selection architecture.
