...
The Map treats post-decoherence selection as its primary response to the [[timing-gap-problem|timing gap]]: decoherence destroys quantum superpositions in warm biological tissue within femtoseconds, which seems far too fast for consciousness to interact with them on plausible neural timescales. Pre-decoherence frameworks remain live under contested coherence-time estimates; post-decoherence selection may sidestep the question entirely, though at a cost in falsifiability addressed below.

## The Core Distinction

The Map's framework—a form of [[interactionist-dualism]]—requires that consciousness causally influences physical outcomes ([[tenets#^bidirectional-interaction|Bidirectional Interaction tenet]]). Two axes organise the options. The first is *temporal direction*: [[time-symmetric-selection-mechanism|retrocausal selection]] biases past quantum events so the downstream chain produces the desired neural pattern, while forward-in-time selection acts on possibilities as they resolve. The second axis is *collapse stage*, and it is meaningful only within the forward branch: selection can target a state before or after [[decoherence|decoherence]] has run. This article follows the forward branch and treats the collapse-stage axis as its main organising distinction; the retrocausal pathway is the foil against which the forward case is built. The forward route therefore splits in two:
...
The argument's force depends on consciousness's access to a distinction that is *FAPP-invisible* (For All Practical Purposes) to local observers: improper and proper mixtures produce identical predictions for any physical apparatus with finite resources. Consciousness would therefore need access to information no embedded detector can have—naked dualism, consistent with the [[tenets#^dualism|Dualism]] tenet. The FAPP gap is not closed by the framework; it is the gap the framework occupies. A critic may reasonably read the same FAPP-invisibility as grounds for denying that anything occupies the gap at all; the evidence does not adjudicate between the two readings.
...
A third approach treats selection as metaphysical rather than physical. The post-decoherence state represents a space of real but unactualised possibilities; consciousness does not interact with a physical system but *actualises* one element into definite experience. This echoes [[process-philosophy|process philosophy's]] concrescence and resonates with Kastner's possibilist transactional interpretation (though Kastner's mechanism is physical emitter-absorber handshakes, not consciousness-mediated).
...
Pre-decoherence frameworks stand or fall on whether macroscopic neural superpositions can exist. Tegmark (2000) calculated decoherence times of 10⁻¹³ to 10⁻²⁰ seconds for neural microtubules. Hagan et al. (2002) challenged his parameters and obtained 10⁻⁵ to 10⁻⁴ seconds under favourable assumptions, but their results remain unreplicated. Avian magnetoreception's microsecond-scale spin coherence shows evolution can sustain coherence at warm-tissue scales, but receptor-protein spin coherence is far from the macroscopic superpositions pre-decoherence selection requires. The [[quantum-neural-timing-constraints|detailed timing analysis]] surveys the landscape: even favourable estimates leave a gap of several orders of magnitude.

Post-decoherence selection may sidestep the question. If consciousness operates on the decohered mixture rather than on pre-decoherence superpositions, coherence survival becomes irrelevant. The trade-off is falsifiability: pre-decoherence models make concrete predictions about quantum coherence at neural timescales; post-decoherence models operate at the interpretive level, where the [[testing-consciousness-collapse|experimental programme]] has less traction. This cost is paid, not avoided.

A more thoroughgoing dissent (Duch 2005, 2019) reads the same decoherence facts as eliminating any quantum role, not relocating it: classical neurodynamics supplies determinacy without any improper-mixture problem to solve. What excludes this closure is the [[tenets#^dualism|Dualism tenet]], not anything internal to the mechanism—the framework here is a consequence of the dualist commitment, not an independent argument for it.
...
**Alignment with established frameworks (moderate).** Pre- and post-decoherence variants draw on orthodox quantum-foundations resources. The retrocausal pathway depends on more speculative frameworks like the transactional interpretation, a minority position in quantum foundations.
...
The [[tenets#^occams-limits|Occam's Razor Has Limits]] tenet cautions against dismissing either pathway prematurely. The improper mixture distinction shows the problem of outcomes is real; the [[time-symmetric-selection-mechanism]] remains a live alternative resolving certain puzzles (particularly the Libet timing problem) that forward-in-time models must address through other means. The [[forward-in-time-vs-time-symmetric-selection|side-by-side cost accounting of the two routes]] reads off what each owes without adjudicating between them—the Map holds both as live alternatives, not rivals.
