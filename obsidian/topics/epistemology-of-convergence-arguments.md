...
The most powerful objection to convergence-as-evidence holds that shared human cognitive architecture explains the patterns better than shared access to truth.
...
Two observations limit mysterianism's force. First, cognitive closure does not favour physicalism — if we cannot grasp the solution, we cannot know whether it is reductive or non-reductive. Mysterianism entails agnosticism, not materialism. Second, the specific structure of the convergence — traditions independently identifying the same *kind* of gap (between structural description and qualitative character) — is more naturally explained by a real feature of consciousness than by a generic cognitive limitation. A mere limitation would be expected to produce variable philosophical errors, not convergence on a structurally precise diagnosis.

## What Divergence Reveals

The epistemological analysis gains depth from examining where traditions *diverge*. If convergence is significant, divergence should be equally informative.
...
A deeper objection holds that *all* these arguments share a common root: the intuition that there is "something it is like" to have an experience. If this intuition is a cognitive artefact, every argument collapses simultaneously. The response is that phenomenal experience is not merely an intuition *about* something; it is the datum itself. Pain's felt character is not a belief about pain that could turn out to be mistaken — it *is* the phenomenon under investigation. Calling it a "cognitive artefact" presupposes that a physical process generates an illusion of experience, but an illusion of experience *is* an experience. The supposed common root is therefore not a shared assumption that could be debunked but the subject matter itself.
...
The [[tenets#^occams-limits|Occam's Razor Has Limits tenet]] connects directly: the appeal of physicalism rests partly on parsimony, but parsimony is a tiebreaker between theories of equal explanatory power, not a trump card that overrides convergent evidence. Dismissing cross-cultural convergence as mere cognitive bias assumes we understand human cognition well enough to explain the pattern away, but the empirical evidence on [[intuitive-dualism]] suggests the pattern is not reducible to innate cognitive tendencies. When multiple independent arguments favor a conclusion, simplicity alone is insufficient to reject it.
