...
description: "A design space, not an experiment: what a Sorkin-Δ third-order-interference test translated to neural tissue would require, why the optical-to-brain translation is non-trivial, and which of Stapp-QZE, Orch-OR, and probability-bias channels it could constrain."
created: 2026-06-03
modified: 2026-06-03
human_modified:
...
**Stapp's quantum Zeno channel.** [[stapp-quantum-mind|Stapp's]] mature proposal is that the mind selects by rapid repeated attention, holding a brain observable in an eigenstate against unitary decay. Read literally, this changes outcome frequencies as a function of attention rate — but a measurement schedule alters survival probabilities relative to free evolution, a second-order effect, while every probability it yields remains a Born probability. The [[quantum-zeno-effect|Zeno effect]] is a consequence of the standard formalism, so the third-order residue stays exactly zero at every attention rate. A brain Sorkin-Δ-analogue is therefore structurally silent against literal Zeno, as it is against the weaker reading on which mind "chooses the observable, not the result." The discriminator that does exist is different in kind: the seventh falsifier the Map's [[stapp-quantum-mind]] entry lists for Stapp's model, a non-linear rise in selection probability as observation rate approaches the Zeno-freezing regime, against the smooth saturation a classical Hebbian account predicts. That test inherits Tegmark's (2000) decoherence scale — attention events must outpace decoherence (10⁻¹³ s on Tegmark's estimate, 10⁻⁵–10⁻⁴ s on Hagan-Hameroff-Tuszyński's) — but it measures a rate-response curve, not a Sorkin residue.

**Penrose-Hameroff Orch-OR.** Orch-OR is explicitly non-standard: objective reduction breaks unitarity at the Diósi-Penrose threshold τ ≈ ℏ/E_G, where E_G is the gravitational self-energy of the superposed mass distribution. Breaking unitarity is not the same as breaking the quadratic form, though. The Diósi-Penrose model in its formal statement — the version the Gran Sasso underground experiment tested and excluded in its parameter-free form (Donadi et al. 2021) — evolves the density matrix by a linear master equation, as every consistent Markovian collapse model must to avoid superluminal signalling. A state represented by a density matrix links at most two paths coherently, so the formal collapse dynamics predicts ε = 0 at every mass scale. Its signature is second-order: interference visibility lost at the threshold mass-time scale, which is what [[penrose-gravity-induced-collapse-empirical-prospects|MAQRO-class interferometry]] looks for, not a third-order residue. Only Penrose's further claim that the OR event selects "non-computable" outcomes could depart from Born statistics, and that claim has no quantitative form a residue could be predicted from. A brain Δ-analogue therefore bears on Orch-OR only if that selection element is ever specified as non-quadratic — and even then only if the superposed mass reaches the threshold within a coherence time, which is the Tegmark dispute again.

**Probability-bias channels.** A channel slightly outside the [[selection-only-channel|corridor]] admits a small per-event deviation δ that reweights the outcome distribution (δ is a bias size, distinct from Sorkin's ε). The empirical ceiling at non-brain scales is roughly δ ≈ 10⁻⁴ from RNG meta-analysis, trending toward the null under preregistered Bayesian work; brain-internal deviations could be smaller still, and exponentially-suppressed weightings can be tuned below current detection while producing macroscopic cumulative effects. A reweighting of Born probabilities need not produce any third-order term, so a Sorkin-Δ-analogue bears only on bias channels that break the quadratic form, and even then only if it reached sub-RNG-detection precision — below 10⁻⁴ per event in the brain-internal regime — which is well beyond any first-generation instrument. A null at 10⁻⁴ would foreclose only the most detectable bias models and leave the sub-threshold ones, and the strict [[channel-class-taxonomy|selection-only corridor]], untouched: the corridor predicts ε = 0 by construction, so the optical-level null it already passes is exactly what it expects.

The pattern across all three: a brain Sorkin-Δ-analogue is informative only against readings that break the quadratic form — detectable non-quadratic probability-bias, at precisions no instrument yet reaches, and Orch-OR's non-computable selection, if it were ever specified as non-quadratic. It is structurally silent against the strict corridor, against literal Zeno, and against the formal Diósi-Penrose collapse dynamics, none of which leaves a third-order residue for a Δ-test to catch. That silence is the honest cost of the corridor reading, not a strength.
