...
**A high-cost target channel.** The perturbation should preferentially load the autonoetic channel — the channel that swings most in recovery order and so carries the largest predicted direction effect — by targeting a region the autobiographical-retrieval literature implicates in re-experiencing while sparing the regions that route noetic recognition.
...
The caution is sharper and must be stated where the design is, because it bounds what a positive result could claim. Proekt and Hudson (2018), in the *British Journal of Anaesthesia*, argue that whole-organism neural inertia need not imply distinct induction and emergence machinery at all: a *single* bistable system driven by stochastic noise reproduces the hysteresis, which then *collapses over time* as noise drives the system toward a common dose-response curve. On their account, induction and emergence can converge on the same underlying mechanism while still presenting an apparent asymmetry over realistic timescales. The design must therefore not treat any observed threshold separation as automatically favouring a direction-dependent architecture: a stochastic-bistable production model predicts hysteresis without any direction-specific channel routing. This is why the *discriminating* read-out is not hysteresis-presence but the **cross-channel ordering** under direction reversal — whether the channels recover in a *different order* than they fail, which the single-bistable-system model, being direction-blind in its ordering, does not predict even though it does predict threshold separation. Conflating "hysteresis exists" with "the interface reading is favoured" would be exactly the inflation the [[evidential-status-discipline|evidential-status discipline]] exists to catch.
