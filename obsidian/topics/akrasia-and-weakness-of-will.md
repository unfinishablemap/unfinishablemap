...
description: "Acting against one's own better judgement: Socrates' denial, Aristotle, Davidson, and Holton, read by the Map as one selection-model lens on the will—not a proof of dualism."
created: 2026-07-09
modified: 2026-07-09
human_modified:
...
- **Socratic denial** (Plato's *Protagoras*): genuine akrasia is impossible; apparent cases are really ignorance or miscalculation.
- **Aristotelian diagnosis** (*Nicomachean Ethics* VII): akrasia is real but is a failure to *activate* knowledge one possesses.
- **Davidsonian resolution** ("How Is Weakness of the Will Possible?"): akrasia is genuinely possible, intentional action against an all-things-considered judgement, and irrational without being incoherent.
...
## Davidson: The Modern Locus

Donald Davidson's essay "How Is Weakness of the Will Possible?" is the modern point of reference. (First published in Joel Feinberg's edited collection *Moral Concepts*, Oxford University Press—catalogue records disagree between 1969 and 1970 for that volume; the stable citation is the reprint in Davidson's *Essays on Actions and Events*, Essay 2, pp. 21–42.) Davidson's achievement is to show that akrasia is possible without being incoherent, thereby answering the Socratic denial on its own terms.

He first sharpens the definition. An agent does *x* incontinently when: (1) *x* is done intentionally; (2) the agent believes some alternative *y* is open; and (3) the agent judges that, all things considered, it would be better to do *y* than *x*. The puzzle is condition (3): how can the agent act against a judgement of what is better *all things considered*?

Davidson's resolution turns on distinguishing kinds of evaluative judgement. **Prima facie** judgements ("in light of reason *r*, *a* is better") and **all-things-considered** judgements are *conditional*—they relate an action to a body of reasons. An **all-out** (unconditional) judgement—"*a* is better," full stop—is what actually issues in action. The akratic, on this analysis, reaches the all-things-considered judgement favouring *y* but forms the all-out judgement favouring *x*, and acts on the latter. The action is intelligible as intentional behaviour yet *irrational*, because the agent has detached the judgement that moves them from the judgement supported by all their reasons.

Davidson names the norm the akratic violates the **principle of continence**: perform the action judged best on the basis of all available relevant reasons. What marks akrasia as irrational, in his words, is that the agent has no *reason* for the gap: "if we ask what is the agent's reason for doing [b] when he believes it would be better...to do another thing, then the answer must be: for this, the agent has no reason" (*Essays on Actions and Events*, p. 42). Crucially for how the Map should handle this material, Davidson renders akrasia *possible but irrational* with no appeal to any non-physical cause. The gap between all-things-considered and all-out judgement is a structural feature of practical reasoning, statable in wholly naturalistic terms.

## Synchronic and Diachronic: Holton's Reconception

Davidson's akrasia is *synchronic*: at a single time, the agent's present action conflicts with their present better judgement. Richard Holton's *Willing, Wanting, Waiting* relocates the failure to a *diachronic* axis and, in doing so, detaches "weakness of will" from acting-against-best-judgement altogether.
...
- The **Socratic denial** contests whether the datum is even genuine. If Socrates is right, there is no "chose X while judging Y better"—only "misjudged Y." The Map's lens presupposes a datum whose reality is itself disputed.
- **Davidson** explains akrasia as *possible but irrational* using only the structure of conditional versus unconditional judgement—no non-physical cause enters.
- **Holton** explains it as ordinary, empirically studied willpower failing to hold a resolution—again, no dualist commitment.

So the Map's contribution here is an *interpretation* of a shared phenomenon, not evidence for its metaphysics. A physicalist can accept every word of the Aristotle, Davidson, and Holton sections and read the selection language as a functional description of neural subsystems. What the Map adds—that the controller is genuinely non-physical—rests on its independent arguments (the hard problem, the conceivability argument), not on akrasia.

**[[tenets#^bidirectional-interaction|Bidirectional Interaction (Tenet 3)]]**: The selection model reads akrasia as a case where the conscious controller's selection diverges from the agent's evaluative state—relevant to *what* the interface selects and how faithfully it tracks reasons. Whether that divergence needs a non-physical selector is exactly what akrasia does *not* settle; the phenomenon is neutral between the Map's reading and the naturalistic ones. This bears on [[moral-responsibility|moral responsibility]] only glancingly: whether akratic acts are free and blameworthy is a further question the Map defers to its dedicated treatment.

**[[tenets#^occams-limits|Occam's Razor Has Limits (Tenet 5)]]**: The temptation is to declare one analysis "simplest"—Socratic reduction to ignorance, or Davidson's minimal apparatus. The Map's discipline here is to resist that move: the descriptive facts underdetermine the analysis, and simplicity is an unreliable guide when the phenomenon's very existence is contested. Holding the four positions open, rather than collapsing to the tidiest, is the honest register.

## Further Reading

- [[topics/free-will]] — The Map's broader treatment of agency and authorship
- [[control-theoretic-will]] — Consciousness as a low-bandwidth controller that gates, stabilises, and steers
