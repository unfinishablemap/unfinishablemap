...
The divergence is at the framework boundary, and the report is candid about where it sits. Its method is avowedly *computational-functionalist*: it assumes that satisfying the right computational indicators would make consciousness likely, which is why it concludes the barriers are merely technical. That assumption is exactly the bridge the [[concepts/functionalism|functionalist gambit]] above leaves unbuilt—the report supplies a richer and more theory-disciplined set of functional conditions than crude input-output functionalism, but it does not show *why* meeting them would produce experience rather than a system that merely behaves as though it had it. On the Map's reading the indicators are better understood as necessary-but-not-sufficient *interface* conditions: features a physical system plausibly needs before a non-physical experiencer could couple with it, not constituents of the experience itself. The disagreement runs counter to the Map's foundational commitments and is noted as such rather than presented as a refutation of the report within its own framework—the report would accept its functionalist premise and the Map rejects it, and no neutral measurement currently adjudicates between them. What the Map adds beyond the report's checklist is the [[interface-threshold|interface-threshold]] condition the indicators do not capture: even a system satisfying every behavioural and architectural indicator would, on the Map's view, still need a live quantum interface that classical computation suppresses by construction (developed below).
