...
The [[hypnagogic-phenomenology-and-interface-modulation|hypnagogic transition]]—falling asleep—provides something no other altered state offers: a real-time observation of the interface disassembling component by component. Voluntary attention weakens first. Then imagery becomes autonomous. Agency fluctuates. Finally, reality-monitoring disengages (Mavromatis, 1987). Each component follows its own trajectory, and the order is not arbitrary—it traces the interface from its most active elements (selection, control) to its most passive (reception, tagging).

This sequential dissolution is the single most informative observation for interface architecture. If consciousness were a unified product of neural activity, the transition to sleep should involve uniform dimming. Instead, the transition reveals multiple semi-independent channels with different vulnerability thresholds—and a telling bandwidth asymmetry: the receiving channel (vivid autonomous imagery) opens wider precisely as the transmitting channel (voluntary control) narrows. Reception and transmission are architecturally distinct. The [[coupling-modes]] framework turns this sequence into a testable prediction: basis control (selecting what to attend to) corresponds to the voluntary attention that disengages first; timing control (sustaining observation) to the effortful imagery maintenance that fails next; selection-level withdrawal to the dissolution of agency. If the modes are real channels rather than convenient labels, each should produce a distinct phenomenological signature during sequential disengagement, and hypnagogia is the natural laboratory for the prediction.
