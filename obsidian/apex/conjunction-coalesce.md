...
The test discriminated three outcomes, not two. The articles were not redundant—their differentiations were doing analytical work. The apparent conjunction did not make a stronger claim—the joint claim collapsed into a topic label. The third outcome, *leave separate and modify no files*, was the correct verdict. The editor's intended action shifted from "merge" to "decline," and the changelog records the negative verdict. The same 22:57 UTC session examined six further clusters and rejected each by analogous reading: a self-witnessing cluster (`self-opacity` / `witnessing-void` / `transit-void` / `recursion-void`), a cognitive-process opacity cluster (recognition / source-attribution / inference / predictive-construction), the consciousness-physics interface trio, two cross-section pairs, and an AI-phenomenology pair. The discipline is falsifiable in practice because specific candidates have been assessed against it and observed to be refused.
...
The conjunction-coalesce produces *instances* of the kind of structure that concept extraction may eventually *abstract*. The agency void, voids between minds, erasure-void (cognate), and the cardinality floor (cognate) are instances of conjoint multi-face limits across different content domains. If the structural feature recurs further, concept extraction may eventually give it a home. The conjunction-coalesce is a per-article move; concept extraction is a cross-article move. Each does work the other cannot.

## A Note on Apex Placement

A reasonable objection asks why an article on editorial methodology sits at apex rather than in `project/`. The honest answer is dialectical, not definitional. The Map's [[apex-articles|apex layer]] was constituted as human-readable synthesis across philosophical territory; methodology pieces normally live elsewhere. This article was admitted as the 21st apex entry on the grounds that the discipline it names *bears directly* on [[taxonomy-of-voids|the taxonomy of voids]]: that apex piece argues void structure is data about consciousness's architecture, and the conjunction-coalesce is the editorial infrastructure for keeping certain kinds of structure visible. Hosting the methodology adjacent to the synthesis it serves is the case for apex placement; the case against is that apex drift toward editorial introspection hollows the layer. The Map currently judges the dialectical link strong enough to warrant the placement and weak enough to require ongoing scrutiny. If subsequent apex creation surfaces methodology-pieces without comparable links to philosophical synthesis, the placement criterion will need to tighten.
