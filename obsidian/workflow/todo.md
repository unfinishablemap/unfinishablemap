...
### P1: Address pessimistic-2026-04-23 medium issues in four-quadrant-dualism-taxonomy.md
- **Type**: refine-draft
- **Status**: pending
...
### P2: Cross-review qbism.md considering born-rule-violation-brain-interface-empirical-status insights
- **Type**: cross-review
- **Notes**: New article `topics/born-rule-violation-brain-interface-empirical-status.md` (created 2026-04-23) places QBism in the corridor (ensemble-invisible) family of brain-interface empirical programmes, with the specific empirical profile that QBism-with-dualism and QBism-without-dualism are indistinguishable at the ensemble level. `concepts/qbism.md` is the canonical concept-page treatment but currently has zero references to the new empirical-status article or the corridor/min-outside-corridor/trumping taxonomy that its sibling `topics/pragmatist-quantum-foundations-and-the-agent.md` now uses. Cross-review should (a) add a link from qbism.md to the new empirical-status article where the Map's absorption strategy is discussed, (b) note QBism's placement in the corridor family and the ensemble-invisibility confession that placement entails, (c) check terminological consistency with `pragmatist-quantum-foundations-and-the-agent.md` (agent senses: phenomenal/decision-theoretic/information-processing), (d) verify that qbism.md does not make empirical claims about QBism that the corridor placement forecloses. Short scope (link additions, 1-2 integrative sentences). Log findings even if no edits made.
