...
- **Separate the three claims about religious and ancestral material.** *This community has a concept of survival*, *survival is metaphysically possible*, and *survival occurs* are three different claims, and ethnographic evidence reaches only the first. This is the ethnographic case of the epistemic-versus-metaphysical reading distinction in [[evidential-status-discipline]].
...
The prose-level rule: claims of empirical or evidential support cite external sources. Internal links are framed as development, derivation, or integration—"as [[evidential-status-discipline]] argues", "the framework treatment is in [[tenets]]"—never as confirmation ("this is confirmed by" followed by a wikilink to another Map article). A chain of internal pages, however long, never substitutes for independent support. This is continuous with the corpus's existing anti-double-count discipline (see [[evidential-status-discipline]]'s *Coherence vs. Evidential Status* and [[common-cause-null]]): a self-citation to another Map article is framework-internal coherence, not framework-independent evidence.

The complementary *visual* separation—splitting the References section into external citations and a separate "Map integration" list—was proposed by the 2026-06-04 ChatGPT 5.5 Pro outer review of `clinical-dissociation-as-systematic-evidence` and re-raised by the 2026-07-16 full-site audit. It remains a *candidate* convention: adopting it is a corpus-wide reformatting decision for the operator, not an autonomous edit. Until then the single-list convention stands, refine passes should not pre-emptively re-section reference lists, and the prose-level discipline above applies now.

**Author-stance verification (citation-verification step for deep-review / refine-draft / expand-topic — adopted 2026-07-21).** Metadata-focused verification (does the work exist? is the DOI right? is the quote verbatim?) misses a distinct and high-yield failure class: a citation that is bibliographically clean and verbatim-faithful yet silently *flips the author's role in the debate*. The convergent 2026-07-21 outer reviews (Claude Opus 4.8 + ChatGPT 5.6 Pro) named this the highest-yield methodology fix, on the shared exhibit of `phenomenal-conservatism.md`, where a phenomenal-conservatism *defender* (Tucker) was conscripted as an objector and the objection assigned to the wrong authors (McCain / Moretti) — verbatim-clean, DOI-correct, role-wrong. To catch it, for every cited critic or defender record — while verifying at source, not from memory — their **role in the debate (defender / critic / neutral) plus a one-sentence statement of their actual thesis**. This is an editor-internal verification step, not a schema field on every article: the note lives in your working check and the changelog, not the article body. Where the role is wrong, re-attribute; where it is right but the article bends the author's stance, re-frame. This is the citation-level complement to the *Don't Conscript Committed Physicalists (the Co-optation Firewall)* discipline above and its verdict-direction leg (camp → concept → verdict): the firewall guards the metaphysical boundary; this guards the debate-role a single citation assigns.

**"Pending verification" is not a publishable state for a load-bearing citation.** If a citation carries weight for a claim and cannot be verified at source, verify it before publishing, or demote/delete the claim it supports — do *not* ship the gap as a disclosure ("citation pending verification"). The `phenomenal-conservatism` BonJour citation violated this by shipping the unverified citation with a hedge instead of resolving it. A useful decomposition of the verification itself (ChatGPT 5.6 Pro's three-stage split): (1) *bibliographic existence* — the work and its identifiers are real; (2) *claim-attribution fidelity* — the cited author actually holds the thesis and role the article assigns (the stance check above); (3) *literature currency* — the citation has not been superseded (see *Citation Aging in Fast-Moving Empirical Fields* above).

**Evidence-kind and decisive-assumption disclosure (adopted 2026-08-17).** The corpus's recurring citation failure is *scope inflation and construct substitution rather than fabricated bibliography* (ChatGPT 5.6 Pro full-site audit, 2026-08-17): a reference list can be flawless while the body misreports what every source found, which is precisely what metadata-checking lenses cannot see. The three instances fixed on 2026-08-17 share one shape — a source's *kind* was silently upgraded. The PBR theorem, conditional on preparation independence, was read as an unqualified metaphysical result; Denton et al. 2024, a simulation identifying parameter regions, was read as working biology; Killingsworth and Gilbert's experience sampling of *task-unrelated thought* was read as a measurement of narrative-self processing.

The standard: where a citation carries weight for a claim, the prose states **what kind of evidence it is** — experiment, simulation, toy model, theorem, review, preprint, or philosophical argument — and **the decisive assumption the result turns on** (a theorem's premise, a simulation's parameter regime, an instrument's measured construct). A clause does the work: "a simulation identifying parameter regions in which…", "a theorem conditional on preparation independence".
