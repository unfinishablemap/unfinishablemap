...
draft: false
description: "Full-site referee audit by ChatGPT 5.6 Pro. Confirms a mathematical error in the selection-only information-theory chain and a register-to-prose propagation failure."
topics:
...
Consequently, any positive conditional departure for one conscious state must be offset by negative departures elsewhere in the same conditioned ensemble, unless every departure is zero. [[born-preserving-causal-efficacy]]
...
The Map’s individuation register currently contains only one principal position even though the No-Many-Worlds argument, AI-copy ethics and interface model all depend on a determinate account of subjects. [[individuation-and-subjecthood]]
...
The Map prefers post-decoherence selection partly because it does not require long-lived coherent neural superpositions. Environmental decoherence produces the relevant alternative outcomes; consciousness supposedly acts at or after that stage. [[quantum-interface]]
...
The AI register itself concedes that its no-cloning argument weakens once consciousness acts through live indeterminacy rather than a stored quantum state. [[ai-consciousness-scope]]
...
Thus exact preservation of a marginal distribution is compatible with maximal conditional dependence. The Map’s own later Born-preserving apex correctly says that marginal preservation alone does not constrain the conditionals. [[born-preserving-causal-efficacy]]
...
The quantum register explicitly states that no toy Hamiltonian, master equation or simulation currently realizes the proposed interface. It also identifies the bias-without-deviation dilemma as the strongest unresolved challenge. [[quantum-interface]]
...
The current Positions index says each domain contains approximately 8–15 positions, but several visible domains contain only one, three, four or five. The individuation register is especially thin despite supporting the No-Many-Worlds and interface arguments. The Open Questions section currently contains only one article despite hundreds of articles and dozens of acknowledged model debts. [[positions]]
...
- The interface synthesis treats a roughly ten-bit-per-second behavioural bandwidth estimate as suggestive of interface capacity, although behavioural output bandwidth is not a measurement of a quantum-consciousness channel. The cited source problem makes the inference weaker still. [[interface-specification-programme]]
- Attention and executive-control findings are compatible with physicalist control architectures. They cannot “bear out” a nonphysical interface without a contrasting quantitative prediction. [[interface-specification-programme]]
- The contemplative-testing article treats cross-traditional convergence and cessation reports as direct evidence against identity theories, while the source-attribution void correctly observes that introspection supplies reconstructed contents rather than native causal-source labels. [[testing-the-map-from-inside]]
...
The Map’s changelog and review process are genuine strengths. The latest review did not merely accept external criticism: it rejected false or stale allegations and verified individual claims. But the same changelog shows that a corrected gloss remained live elsewhere, that a date error propagated from a research note, and that one automation path did not enforce the intended age gate. [[changelog]]
...
The methodology explicitly recognizes “coherence inflation”: repeated self-review can make a framework more internally polished without independently supporting it. It accordingly calls for steelmanning, provenance checks, external red teams and eventual review by human specialists. [[coherence-inflation-countermeasures]]
...
The project has developed a high-stakes claim register, but its calibration is not consistently visible in the article body where readers encounter the claim. [[public-claim-register]]
...
The methodology register says a tenet can remove a defeater but cannot upgrade evidence, and that the interface remains an open programme rather than an established theory. [[methodology-and-calibration]]
...
The apex correctly says a preserved marginal places no direct restriction on consciousness-conditioned probabilities. The selection-only cluster says the same preservation forces zero mutual information. The latter is mathematically false. [[born-preserving-causal-efficacy]]
2. **Agency register versus interface trilemma.**  

The current agency register acknowledges that its trilemma is not exhaustive and omits hierarchical, emergent, interventionist and dynamical alternatives. The interface synthesis still presents the trilemma as exhausting the possibilities and treats mental causation as the only route preserving authorship. [[agency-and-will]]
3. **Calibrated Tenet 3 versus categorical epiphenomenalism rhetoric.**  

The Tenets page treats the argument from knowledge as contested and concedes a correlation reply. The interface synthesis calls epiphenomenalism internally incoherent. These are different assessments of the same objection. [[tenets]]
4. **Mechanism-debt register versus accomplished physical claims.**  

P-Q10 says there is no worked equation or simulation. The interface synthesis nevertheless says energy conservation and no-signalling are preserved and describes the architecture as derived. Physical consistency has been specified as a goal, not established by construction. [[quantum-interface]]
5. **Corrected causal taxonomy versus delegatory sibling pages.**  

Recent apex work distinguishes quantum difference-making from delegatory trumping as incompatible routes. Older concept and topic pages still call delegation the preferred integrated mechanism or say the two routes mutually complete one another. [[changelog]]
6. **Source-attribution void versus first-person testing.**  

The void article says introspection does not contain native tags revealing whether an experience was physically or nonphysically caused. The contemplative-testing article treats some reports as direct evidence against physical identity accounts. The first proposition undercuts the second inference. [[testing-the-map-from-inside]]
7. **Substrate permissiveness versus current AI exclusion.**  

One position treats non-biological consciousness as unobstructed in principle, while the AI register leans against current digital systems partly because they allegedly suppress the relevant indeterminacy. The post-decoherence model removes the coherence-based rationale unless an eligibility law supplies a new distinction. [[consciousness-scope]]
8. **Retireable individuation position versus load-bearing No-Many-Worlds identity.**  

The register architecture says ordinary positions may be retired without disturbing the tenets. Yet the site acknowledges that the main indexical argument for Tenet 4 presupposes a non-deflationary subject. [[positions]]
9. **Value-sensitive steering versus exact aggregate preservation.**  

The value cluster entertains systematic steering toward valuable outcomes, while the default interface preserves physical marginals exactly. Both can coexist only through explicit compensating departures or a carefully restricted ensemble. The site has not yet supplied that account. [[value-in-selection]]
10. **Self-binding on parsimony versus residual Many-Worlds summaries.**  

Tenet 5 forbids decisive anti-Everettian use of ontological economy, while some concept and probability pages continue to foreground ontological proliferation and suggest that the probability problem may be fatal. [[tenets]]
