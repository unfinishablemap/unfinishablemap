...
**Previous review**: [[deep-review-2026-07-15-time-symmetric-selection-mechanism|2026-07-15]]
...
1. **The photosynthesis over-claim is corpus-wide.** Six further live files assert essentially the same superseded claim: `topics/evolutionary-case-for-quantum-neural-effects.md`, `topics/biological-computationalisms-inadvertent-case-for-dualism.md`, `concepts/entanglement-binding-hypothesis.md`, `concepts/prospective-memory.md`, `concepts/biological-computationalism.md` (plus a refinement-log sidecar for `non-temporal-consciousness`, which should not be touched). Not swept in this pass — reported for task minting. Note that `find_superlative_claims` will not surface these; grep `photosynthetic energy transfer` and `photosynthe.*coheren` instead.
