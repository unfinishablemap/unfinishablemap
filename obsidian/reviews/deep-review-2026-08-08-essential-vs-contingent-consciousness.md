...
> **RESOLVED 2026-08-08 (refine-draft).** The book was obtained and the question is settled: **"zero-person perspective" is Metzinger's own term**, used **45 times** in *The Elephant and the Blind* (2024), where he introduces it explicitly — *"Consciousness itself can be experienced from what this book terms the 'zero-person perspective'"* and *"what in chapters 3 and 29 I will call taking the 'zero-person perspective'"*. Confirmed by three independent extractions: the MIT Press open-access deposit on archive.org (`mit_press_book_9780262377287`, 271k words), a second independent scan of the same book, and the author's own self-archived PDF at `thomasmetzinger.com` run through `pdftotext -layout`. (In that third extraction a naive grep returns **0** because the PDF carries soft hyphens inside `zero-­person`; strip U+00AD and it returns 45. A bare zero was an extraction artifact, not absence.) Metzinger does **not** cite Taguchi. The finding above — that the phrase is absent from the **2020 paper** — remains correct; the error was only the *citation year*, so the fix is re-pointing to 2024, **not** removal. The removal at `essential-vs-contingent-consciousness` L80 was therefore over-cautious, though the replacement wording is accurate Metzinger vocabulary and has been left in place. **Do not mass-delete this phrase.**
