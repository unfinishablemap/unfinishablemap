...
- **Severity**: **Medium**
- **Problem**: Same shape as Issue 2. The body says *"Block's Inverted Earth scenario strengthens the argument…"*. The only Block reference is `Block, N. (1978). "Troubles with Functionalism."` That entry is correct for the China-brain/absent-qualia objection at L149, but Inverted Earth is Block (1990). The Map's own `concepts/inverted-qualia.md` L210 carries the correct entry: `Block, N. (1990). "Inverted Earth." Philosophical Perspectives, 4, 53-79.` One reference entry is being made to cover two distinct arguments from different papers.
...
- **Severity**: **Low-Medium**
- **Problem**: *"Einstein reported general relativity \"felt right\" before he could prove it."* The quotation marks assert a verbatim report from Einstein; no source is given and none exists in the reference list. `reviews/deep-review-2026-06-03-qualia.md` L61 classified it as a *"paraphrase-descriptor (not a sourced verbatim quote)"* — a reasonable internal judgement, but the article's punctuation does not convey it. An LLM fetching this page will read it as attributable verbatim.
...
- **Problem**: Three faults in one paragraph.
  ...
  3. **Broken deferral.** The paragraph closes *"See [[minimal-consciousness]] for detailed treatment."* `concepts/minimal-consciousness.md` returns **zero** matches for slime / Physarum / maze. The reader is sent to a page that does not treat the claim. (The *C. elegans* half of the deferral is sound — that page does discuss it.)
- **Recommendation**: The Map already handles this claim correctly elsewhere. `concepts/functionalism.md` L117 cites Nakagaki et al. (2000), Tero et al. (2010) and Boisseau et al. (2016), and — critically — draws the *disciplined* conclusion instead of the dissociation one: either *Physarum* has some form of consciousness (which most functionalists resist) or additional criteria beyond functional role separate cognitive from conscious systems. Inherit that framing and the citation (`Nakagaki, T., Yamada, H. & Tóth, Á. (2000). "Maze-solving by an amoeboid organism." Nature, 407, 470.` — `functionalism.md` L208). Repoint the deferral to `[[concepts/functionalism]]` or to the minimal-consciousness page only for the worm case.
