...
draft: false
description: "Adversarial review of the applied apex `moral-status-of-edge-cases`, 24 minutes after creation: the possibility→probability discipline holds on the presence side and is never run on the absence side; the calibration surface omits the one axis an applied piece needs."
ai_contribution: 100
author:
...
|---|---|---|---|---|
| P-MS1 | moderate | D | developed | indirect |
| P-MS2 | low | n/a | developed | n/a |
| P-CS2 | moderate | **B** | developed | indirect |
| P-CS3 | moderate | **B** | developed | indirect |
| P-CS4 | moderate | **C** | developed | indirect |
| P-CS5 | moderate | **C** | developed | **none-by-construction** |
| P-SC2 | high | n/a | developed | n/a |
| P-SC3 | moderate | D | **programme** | **none** |
| P-AC1 | moderate | D | developed | indirect |
| P-AS1 | moderate | **B** | developed | indirect |

  Eight of ten are *moderate*. The one *high* (P-SC2) is high about a **debt**, not a verdict, and the one *low* (P-MS2) is a conjunction ceiling. There is no order-of-magnitude confidence spread to sort on.
  ...
  - **P-CS4 is grade C**, not D — and the article itself says so correctly at L90 ("at **moderate credence on grade C**"). The recommendation contradicts the article's own body four sections earlier.
  - **P-CS5 is grade C**, not D (the mechanistic completeness of chemotaxis is independently attested).
  - **P-AS1 is grade B**, not D — outside the marker region entirely ("attaches to the engineering premises only — quantum error correction's isolation of the logical state ... Albash & Lidar 2015").
...
- **Severity**: High
- **Problem**: `grep -ioF "discriminab"` on the article returns **0** (positive control: `grade` returns 20). The article reports credence for nine positions and grade for six, and never reports the axis that tells a decision-maker whether evidence could ever correct the verdict. This matters most exactly where the article is most confident. Recommendation 2 — *"This recommendation is the most confident one here"* — rests on P-CS4 and P-SC3. P-SC3's register line reads **model maturity programme** ("no account exists of what establishes or terminates a pairing") and **empirical discriminability none**. The article reports P-SC3 as "at **moderate credence**" and stops.

  The whole floor section rests on P-CS5, whose discriminability is **none-by-construction**. So the two positions carrying the article's most confident recommendation and its most distinctive section are the two cited positions on which no observation can bear, and a reader is not told.
- **Popper's form of the objection**: recommendation 2 instructs clinicians to treat absence of report, absence of response and absence of neural signature as evidence about the interface rather than the bearer. Since P-SC3's discriminability is *none*, **no** observation can bear on the bearer. That is a defensible position, but an applied piece that recommends acting on it owes the reader the statement that nothing could count against it — and the register supplies the label.
- **Recommendation**: Add the discriminability axis wherever the article already surfaces credence for a position carrying a *decision implication*. Minimum viable fix: one clause in recommendation 2 naming P-SC3's *programme* maturity and *none* discriminability, and one clause in the floor section naming P-CS5's *none-by-construction*. Both are affordable — 1,100 words of headroom to the apex hard gate.
...
- **`concepts/possibility-probability-slippage.md` exists and is not linked**, though it is the corpus's dedicated page for the exact hazard this article is built around, and both `birch-edge-of-sentience-and-the-five-tier-scale` and `ethics-of-consciousness-invertebrate-question` link to it.
...
- **Quote fidelity against register source — CLEARED except one truncation.** Eleven quoted strings checked verbatim against `positions/`: P-SC2's "hard bound on what any applied verdict may assert about subject counts" ✓; P-SC3's "markers date the appearance of correlates of experience, not the establishment of a pairing, and nothing in the corpus licenses reading one off the other" ✓; P-CS5's "mechanistic sufficiency shows a chooser or a subject is *unnecessary*, not that either is *absent*" ✓, "no coupling, nothing chosen, nothing felt" ✓, "undefeated but not positively established" ✓, "the Map endorses it as the reading the behaviour matches while denying it is proven" ✓; P-AC1's *"low probability," not "ruled out"* ✓ and "whatever biology happens to have" ✓; P-AS1's "would remove a defeater, not supply evidence of presence" ✓ and "raw indeterminacy present, interface requirements failed" ✓. Only the P-CS4 quote is cut short (secondary record above).
- **Band attributions — CLEARED on credence, drifted on grade.** Every credence claim in the article matches the register: P-MS1 moderate/D ✓, P-MS2 low ✓, P-SC2 high ✓, P-CS4 moderate/C ✓, P-SC3 moderate ✓, P-CS2 moderate/B ✓, P-CS3 moderate/B ✓, P-AC1 moderate/D ✓, P-CS5 moderate on the in-principle thesis ✓. Only recommendation 4's summary drifts (Issue 1).
- **The 2026-09-16 P-MS1 revision claim — CLEARED.** L92 says the revision "made the dependency explicit: across a non-experiential interval the momentary-locus reading supplies no bearer." `positions/moral-status-calibration-history.md` L39 confirms verbatim in substance, including that it reversed the 2026-08-27 refusal.
- **Wendler 2023 and Feinberg 1974 attributions — CLEARED against the register.** `moral-status.md` L65 records "the equality argument in general form after Wendler 2023"; P-MS1's *Depends on* records "the conation-requirement reply to biocentrism (Feinberg 1974)." (Register-level only — not verified at publisher.)
...
| "even a future architecture built to host open selection 'would remove a defeater, not supply evidence of presence.'" | L120 | Holds — quoted verbatim from P-AS1, not paraphrased |
...
| "every other verdict in this piece rests on grade D or on a registered debt" | rec 4, L134 | False for P-CS4 (C), P-CS5 (C), P-AS1 (B) |
...
| "no quantitative measure of consciousness escapes it" | L70 | P-SC2 adds "unless the measured properties are shown intrinsic or systemic" |
...
| "removes the parsimony-based dismissal of minimal minds" (L124, L153) | Overstates the register — P-CS5 holds the default *undefeated* | "removes parsimony's authority to settle the question, without disturbing its verdict" |
...
- **The foetal section is the best passage in the piece** — P-CS3 dates phenomenality, P-SC3 forbids reading pairing off the same markers, and the article refuses to manufacture the commitment the register withholds.
