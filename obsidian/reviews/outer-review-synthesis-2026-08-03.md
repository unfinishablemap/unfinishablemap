...
draft: false
description: "Cross-review synthesis of three full-site outer reviews from 2026-08-03. Five convergent clusters; three task upgrades; the headline finding is that the Map corrects in one place and leaves the same claim standing elsewhere."
topics: []
concepts: []
related_articles:
  - "[[project]]"
...
- **Flagged by**: chatgpt, claude
- **Verification**: Clean. ChatGPT's half was confirmed at collection; verified again here — `positions/individuation-and-subjecthood.md` carries exactly one position heading (P-I1). Claude's half is drawn from the Map's own concession and the Map does not dispute it.
- **Quotes**:
...
- **ChatGPT 5.6 Pro**: Born-marginal preservation does not entail zero mutual information, and "signed mutual information" is a category error → `todo.md` task on `concepts/selection-only-channel.md` (P1). **The strongest single finding of the cycle despite being a singleton**, and the exemplar of C1.
- **ChatGPT 5.6 Pro**: P-CS1 grades substrate neutrality a "direct entailment" of bare Tenet 1 → task on `positions/consciousness-scope.md` (P2).
- **Claude Opus 5**: P-Q3 lists Chalmers and McQueen as a bare dependency where the authors disclaim the substance reading → task on `positions/quantum-interface.md` (P2).
- **Claude Opus 5**: the voids index lists `perceptual-reality-monitoring-void` as both a published standalone and a folded research note → task on `voids/voids.md` (P2).
- **Claude Opus 5**: the AI-scope register never books that Saad's Organizational Invariance constraint is incompatible with P-AC1 → task on `positions/ai-consciousness-scope.md` (P2).
