---
ai_contribution: 100
ai_system: claude-opus-4-5-20251101
concepts: []
created: 2026-02-04
date: '2026-02-04'
draft: false
lastmod: 2026-02-04 00:00:00+00:00
related_articles: []
title: Tenet Alignment Check - 2026-02-04
---

# Tenet Alignment Check

**Date**: 2026-02-04
**Files checked**: 232 (164 concepts + 68 topics)
**Errors**: 0
**Warnings**: 0
**Notes**: 0

## Summary

All 232 content files in `obsidian/concepts/` (164 files) and `obsidian/topics/` (68 files) were reviewed for alignment with the five foundational tenets. No violations were found. Articles discussing competing philosophical positions (materialism, illusionism, functionalism, epiphenomenalism, many-worlds interpretation, panpsychism) consistently frame them as views the Map critiques rather than endorses. Most files include explicit "Relation to Site Perspective" sections connecting content to specific tenets.

### The Five Tenets Checked Against

1. **Dualism**: Consciousness is not reducible to physical processes.
   - *Rules out*: Eliminative materialism, reductive physicalism, any view treating consciousness as purely epiphenomenal or illusory.

2. **Minimal Quantum Interaction**: The smallest possible non-physical influence on physical outcomes via quantum indeterminacy.
   - *Rules out*: Quantum woo/mysticism, uncontrolled psychokinesis, energy injection by mind, empirically detectable mind-matter interactions.

3. **Bidirectional Interaction**: Consciousness causally influences the physical world.
   - *Rules out*: Pure epiphenomenalism, parallelism without interaction, views where consciousness is "along for the ride."

4. **No Many Worlds**: The many-worlds interpretation of quantum mechanics is rejected.
   - *Rules out*: Treating all quantum branches as equally real, dismissing indexical identity as meaningless.

5. **Occam's Razor Has Limits**: Simplicity is not a reliable guide to truth when knowledge is incomplete.
   - *Rules out*: Using parsimony as a decisive argument against dualism, treating Occam's Razor as more than a defeasible heuristic.

## Errors

None.

## Warnings

None.

## Notes

None.

## Key Observations

### Articles on Competing Positions

The following articles discuss philosophical positions that the tenets rule out, but do so appropriately—framing them as positions the Map critiques:

- **materialism.md**: Explains why physicalism fails; core articulation of why the Dualism tenet is adopted
- **illusionism.md**: Treats illusionism as the strongest challenge to the Map's position while presenting comprehensive responses
- **functionalism.md**: Discusses functionalism's challenges from the Map's perspective
- **epiphenomenalism.md**: Explains the self-stultification problem; explicitly notes the tenets reject this view
- **many-worlds.md**: Presents the No Many Worlds tenet's rejection with detailed arguments
- **panpsychism.md**: Treats panpsychism as an alternative to explore while noting tensions with the Map's framework

### Strong Tenet Alignment Patterns

- Articles on **neuroplasticity**, **contemplative-neuroscience**, and **stapp-quantum-mind** provide empirical support for Bidirectional Interaction
- Articles on **emergence**, **mental-causation**, and **downward-causation** develop the theoretical framework for mind-matter interaction
- Articles on **self-stultification**, **argument-from-reason**, and **reasons-responsiveness** provide argumentative support against epiphenomenalism
- Articles on **indexical-facts**, **self-locating-beliefs**, and **haecceity** support the No Many Worlds tenet
- Articles on **hard-problem-of-consciousness**, **explanatory-gap**, and **knowledge-argument** support the Dualism tenet

## Topics Files Passing All Checks

All 68 topic files pass tenet alignment:

- topics (section index)
- consciousness-and-mathematical-understanding
- quantum-neural-timing-constraints
- decoherence-and-macroscopic-superposition
- quantum-biology-and-the-consciousness-debate
- quantum-measurement-and-subjective-probability
- quantum-measurement-interpretations-beyond-mwi
- chalmers-psychophysical-coupling
- machine-consciousness
- consciousness-and-intelligence
- contemplative-evidence-for-consciousness-theories
- authentic-vs-inauthentic-choice
- epistemic-advantages-of-dualism
- attention-disorders-and-quantum-interface
- why-phenomenal-unity-resists-explanation
- time-symmetric-selection-mechanism
- attention-schema-theory-critique
- integrated-information-theory-critique
- quantum-measurement-and-definite-outcomes
- contemplative-neuroscience-integration
- temporal-structure-of-consciousness
- consciousness-in-simple-organisms
- structure-of-attention
- phenomenology-of-recursive-thought
- vertiginous-question
- eastern-metaphysics-integration
- eastern-philosophy-haecceity-tension
- attention-interface-mechanisms
- delegatory-dualism
- choking-phenomenon-mental-causation
- loss-of-consciousness
- indexical-identity-quantum-measurement
- comparative-consciousness-and-interface-differences
- neural-implementation-specifics
- phenomenal-value-realism-development
- phenomenology-of-evidence-assessment
- eastern-philosophy-consciousness
- consciousness-and-meaning-integration
- conservation-laws-and-mind
- emotional-consciousness
- creativity-and-novel-combination
- ai-consciousness
- phenomenological-evidence-methodology
- first-person-third-person-methodology
- hard-problem-of-consciousness
- quantum-binding-and-phenomenal-unity
- psychophysical-laws-framework
- purpose-and-alignment
- animal-consciousness
- leibnizs-mill-argument
- history-of-interactionist-dualism
- meaning-of-life
- personal-identity
- ethics-of-consciousness
- time-collapse-and-agency
- presentiment-and-retrocausality
- aesthetic-dimension-of-consciousness
- lucid-dreaming-and-consciousness
- argument-from-reason
- phenomenology-of-understanding
- phenomenology-of-intellectual-effort
- consciousness-and-language-interface
- language-recursion-and-consciousness
- meaning-and-consciousness
- william-james-consciousness
- split-brain-consciousness
- death-and-consciousness
- free-will

## Concept Files Passing All Checks

All 164 concept files pass tenet alignment:

- access-consciousness
- agent-causation
- altered-states-of-consciousness
- analytic-idealism
- arguments-against-materialism
- arguments-for-dualism
- attention
- attention-as-interface
- attention-motor-quantum-interface
- attention-schema-theory
- attentional-economics
- autonoetic-consciousness
- baseline-cognition
- bidirectional-interaction
- binding-problem
- blindsight
- brain-interface-boundary
- buddhism-and-dualism
- causal-closure
- cognitive-closure
- cognitive-phenomenology
- collapse-and-time
- combination-problem
- concepts
- conscious-vs-unconscious-processing
- consciousness
- consciousness-and-creativity
- consciousness-and-social-cognition
- consciousness-as-amplifier
- consciousness-selecting-neural-patterns
- consciousness-value-connection
- contemplative-neuroscience
- continual-learning-argument
- counterfactual-reasoning
- coupling-modes
- cumulative-culture
- decoherence
- default-mode-network
- dopamine-selection-interface
- downward-causation
- dreams-and-consciousness
- dualism
- duration
- embodied-cognition
- emergence
- epiphenomenalism
- episodic-memory
- epistemic-emotions
- evolution-of-consciousness
- existentialism
- experiential-alignment
- explanatory-gap
- filter-theory
- functionalism
- global-workspace-theory
- haecceity
- heterophenomenology
- higher-order-theories
- idealism
- illusionism
- implicit-memory
- indexical-facts
- integrated-information-theory
- intentionality
- interactionist-dualism
- intersubjectivity
- introspection
- intuitive-dualism
- inverted-qualia
- jourdain-hypothesis
- knowledge-argument
- libet-experiments
- llm-consciousness
- locality
- luck-objection
- many-worlds
- materialism
- measurement-problem
- meditation-and-consciousness-modes
- mental-causation
- mental-effort
- mental-imagery
- metacognition
- metarepresentation
- mind-brain-separation
- mind-matter-interface
- minimal-consciousness
- moral-responsibility
- motor-selection
- multi-mind-collapse-problem
- multimodal-binding
- mysterianism
- near-death-experiences
- neural-binding-mechanisms
- neural-correlates-of-consciousness
- neural-quantum-coherence
- neurophenomenology
- neuroplasticity
- nihilism-and-existentialism
- objections-to-interactionism
- objectivity-and-consciousness
- observational-closure
- pairing-problem
- panpsychism
- parfit-reductionism
- phenomenal-concepts-strategy
- phenomenal-consciousness
- phenomenal-intentionality
- phenomenal-overflow
- phenomenal-unity
- phenomenal-value-realism
- phenomenological-evidence
- phenomenology
- phenomenology-of-choice
- philosophical-zombies
- philosophy-of-time
- prebiotic-collapse
- predictive-processing
- problem-of-other-minds
- process-philosophy
- prospective-memory
- psychophysical-coupling
- psychophysical-laws
- qbism
- qualia
- quantum-binding-experimental-evidence
- quantum-biology
- quantum-coherence-protection
- quantum-consciousness
- quantum-decoherence-objection
- quantum-indeterminacy-free-will
- quantum-interpretations
- quantum-neural-mechanisms
- quantum-probability-consciousness
- reasons-responsiveness
- reductionism
- relational-quantum-mechanics
- retrocausality
- russellian-monism
- selection-laws
- self-and-consciousness
- self-locating-beliefs
- self-stultification
- semantic-memory
- simulation
- sleep-and-consciousness
- specious-present
- spontaneous-collapse-theories
- stapp-quantum-mind
- substance-causation
- substance-property-dualism
- substrate-independence
- symbol-grounding-problem
- teaching-as-metarepresentation
- temporal-consciousness
- temporal-structure-of-understanding
- theory-of-mind
- time-symmetric-physics
- unity-of-consciousness
- varieties-of-unity
- visual-consciousness
- voluntary-attention
- witness-consciousness
- working-memory