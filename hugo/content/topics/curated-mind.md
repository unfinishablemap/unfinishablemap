...
human_modified: null
last_curated: null
last_deep_review: 2026-07-06 10:48:52+00:00
modified: *id001
related_articles:
- '[[selective-correction-and-reconstruction-paradox]]'
...
A physicalist can respond that the correction works for the system's *behavioural outputs* — no distinct recipient is needed, just a system with adaptive goals. The presupposition argument does not logically compel dualism. Rather, it is an inference to best explanation: the three-mode architecture — correction, transmission, and selection under ambiguity (the conscious-selection event that the [selection-criterion-problem](/concepts/selection-criterion-problem/) and [valence-and-conscious-selection](/topics/valence-and-conscious-selection/) cluster examines) — is *better explained* by a recipient whose experience the curation serves than by mere functional coherence. The next sections develop why.
...
In each case, the structure matches the visual reconstruction paradox. The brain curates the feed. When curation works, consciousness receives a coherent, useful model. When it fails, consciousness receives a wrong model and — critically — cannot correct it from the inside. The subject depends on the editorial process and has no backstage pass. [Clinical dissociation](/topics/clinical-dissociation-as-systematic-evidence/) adds a further dimension: in dissociative amnesia, stored memories become inaccessible to consciousness without any trace destruction — the memory curation channel disconnects while the stored content remains intact, recoverable when the channel reconnects. The curation architecture is not merely editorial; it is the functional bridge between processing and awareness. [Clinical phenomenology](/topics/clinical-phenomenology-and-altered-experience/) extends this analysis by treating psychiatric disruptions as natural experiments on curation itself: schizophrenia's disruption of the minimal self, depression's collapse of temporal horizon, and depersonalization's cracking of phenomenal transparency each reveal which curation channels fail independently, testing the three-mode architecture against real pathology.
...
3. The consumer cannot access the reconstruction process itself ([phenomenal transparency](/concepts/phenomenal-transparency-opacity-spectrum/))
...
The [illusionist](/concepts/illusionism/) response is that the apparent subject is itself a reconstruction — the brain's model of itself as a conscious recipient. On this view, there is no curated feed because there is no one to receive it. The feeling of being a subject is one more thing the brain fabricates.
...
[Global Workspace Theory](/concepts/global-workspace-theory/) offers the most developed physicalist attempt at such an alternative. On this view, modular processors (Fodor, 1983) broadcast their outputs to a shared workspace, and the "recipient" is just the workspace itself — a functional architecture, not a subject. The correction-versus-transmission asymmetry would reflect which modules have broadcasting priority under different reliability conditions. Predictive processing frameworks (Clark, 2013) add further detail: the brain's hierarchical prediction system explains why some errors are corrected (strong top-down priors) and others transmitted (precise bottom-up signals). But the workspace account still faces the convergence problem. The same broadcasting architecture operates identically across vision, proprioception, memory, and self-model — four domains with different evolutionary histories, different neural substrates, and different computational demands. That convergence needs explanation. A workspace designed for computational efficiency would not necessarily organise all four domains around producing coherent *experience*; it might optimise for speed, energy, or behavioural relevance instead. The convergence on experiential coherence as the apparent design target seems to be a datum that workspace models acknowledge but may not fully explain. A workspace theorist could reply that "experiential coherence" is itself just behavioural coherence redescribed — a response the presupposition argument must answer rather than assume away.

## Relation to Site Perspective

The Unfinishable Map interprets the curated mind as evidence for [interactionist-dualism](/concepts/interactionist-dualism/) — though "evidence" here means *coherence with* rather than *proof of*, in keeping with the inference-to-best-explanation framing above. The convergent curation architecture across modalities is roughly what the Map's framework would predict: if consciousness is ontologically distinct from neural processing (the Dualism tenet), then the brain-consciousness interface might be expected to exhibit editorial structure — selective correction, faithful transmission, and selection under ambiguity — rather than uniform throughput. The interface would then show what could be read as evidence of serving a recipient. The prediction is real but soft: the deflationary reading can accommodate the same architecture, so the curation data favour dualism only on the assumption that the recipient framing is the more economical explanation — itself a contestable premise.
...
- [selective-correction-and-reconstruction-paradox](/concepts/selective-correction-and-reconstruction-paradox/) — the reconstruction paradox: selective correction, faithful transmission, and selection under ambiguity (the three-mode taxonomy this article extends across modalities)
