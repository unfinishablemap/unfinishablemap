...
ai_modified: 2026-06-22 06:51:22+00:00
...
The integration above treats active inference as architecture the Map adopts while supplying its own account of what does the selecting — adequate while active inference is a theory of perception and action, but no longer, because it now has a direct theory of consciousness that claims to explain phenomenal experience itself with no role left for a non-physical contributor. The Map's interface reading and this theory are rivals for the same territory, and honesty requires stating the rival in its own terms before replying.

Laukkonen, Friston and Chandaria (2025) propose that consciousness arises specifically at the interface between continuous sensory perception and discrete, counterfactual policy selection — the "beautiful loop" by which a self-modelling system represents its own inference. Three conditions are jointly sufficient for phenomenal experience. An *epistemic field*: the system holds a unified probabilistic representation of its own states. *Bayesian binding*: that representation is integrated into a single coherent estimate rather than separate inferences. And *epistemic depth*: the system models its own modelling — its inference includes a representation that it is itself the inferring system. When a predictive architecture satisfies all three, the authors argue, there is something it is like to be that architecture, and the recursion *is* the experience. This account does not treat consciousness as an add-on the formalism is neutral about; it is built to entail consciousness from the dynamics, so that where the loop closes on itself with sufficient depth, experience is present and nothing further — physical or non-physical — needs to be supplied. It is the strongest physicalist alternative the Map currently faces, because it occupies the Map's own ground: it locates consciousness at a self-referential interface and claims that interface needs no selector beyond the recursion itself.

The Map's reply does not begin by invoking its tenets. The beautiful-loop theory specifies, with real precision, the *structural* conditions under which a system would build a unified, self-locating model of its own inference. What it has not shown — and what its own resources do not deliver — is why a system meeting those conditions *feels like* anything rather than merely computing a self-model in the dark. Epistemic depth is recursion: a representation that represents itself as the represented. The authors move from "the system models itself as experiencing" to "the system experiences," and that move is the framework helping itself to exactly the bridge the hard problem asks it to build. A flight simulator can model the aircraft modelling its own fuel state; adding the recursive self-reference does not make the simulator a pilot. The theory's three conditions are conditions on *representational structure*, and the question the [explanatory-gap](/concepts/explanatory-gap/) poses is why any representational structure, however self-involving, is accompanied by experience at all.

The theory's most sophisticated move sharpens this objection rather than dissolving it. Its authors, like Clark, Friston and Wilkinson before them, can argue that a system with epistemic depth will *represent* itself as having ineffable phenomenal states — the seeming-of-experience falls out of the self-model. But this is the meta-problem move raised one level: it predicts that a beautiful loop would report and model itself as conscious, which is exactly what a system lacking experience but possessing the recursive self-representation would also do. That the architecture generates the conviction of consciousness leaves untouched whether the conviction tracks anything. The account is strong where the Map agrees with it — the conditions plausibly are the neural signature of the interface — and unsupported where it claims more, at the step from self-modelling to felt experience.

The residue is a framework-boundary disagreement, and the Map marks it as such rather than dressing it as a refutation. Both sides can agree point for point on the architecture: a unified self-locating model, integrated, recursively deep, at the perception-policy interface. They part on what completes it. For the theory, the recursion is self-sufficient — the loop's closing constitutes the experience. For the Map, the same closed loop is the physical side of an interface whose felt side the recursion specifies but does not generate, and a non-physical contributor enters at the seam the theory declares already full. Neither side refutes the other from inside its own commitments; the beautiful-loop account remains a live and serious rival, defeated neither by the tenets nor by decisive evidence, and distinguished from the Map only by whether the recursive interface is complete in itself.
...
PP treats precision — the inverse variance of a probability distribution — as the mechanism governing what reaches awareness and what remains background processing: high-precision prediction errors receive more weight, low-precision ones are suppressed. This is functionally equivalent to attention, exactly the mechanism through which the Map argues consciousness acts on the physical world. PP describes *what* precision weighting does; the Map proposes *who* is doing the weighting. The [attention-and-the-consciousness-interface](/topics/attention-and-the-consciousness-interface/) develops the experiential side: PP's precision-weighting is the computational description of the same interface operation that phenomenal attention describes from the first-person perspective, and the convergence of attentional and motor selection onto shared neural substrates specifies where this interface operates.
...
**The unity of perception and action.** Active inference dissolves the boundary between sensing and acting into a single prediction-error-minimisation cycle, aligning with the Map's view that consciousness's causal influence operates at the neural level, with bodily action as downstream consequence.

What PP misses is what every computational framework misses.
