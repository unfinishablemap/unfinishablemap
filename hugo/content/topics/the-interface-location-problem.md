...
Rather than identifying a specific molecular site, some formulations locate the interface at the functional level of attention networks themselves — the frontoparietal systems that implement directed focus. On this view, the relevant question is not "which molecules?" but "which neural systems?", and the answer is: whatever systems implement [willed attention](/concepts/voluntary-attention/).
...
**Difficulties.** Without a molecular mechanism, the proposal is incomplete. Saying "consciousness interfaces through attention networks" pushes the question one level down: which specific processes *within* attention networks are quantum-sensitive? A functional-level account must eventually be grounded in physics to meet the [Minimal Quantum Interaction](/tenets/#minimal-quantum-interaction) tenet's requirements.
...
**3. The attention connection.** The Map proposes that consciousness acts through [attention](/concepts/attention-as-interface/). The interface site must be within the scope of attentional modulation — influenced by the frontoparietal networks that implement directed focus.

**4. The developmental pairing.** The [five criteria](/concepts/brain-interface-boundary/) for a consciousness interface include developmental history. The interface location must be something that develops *with* the organism, not a fixed feature of neural architecture that could serve any consciousness equally.
