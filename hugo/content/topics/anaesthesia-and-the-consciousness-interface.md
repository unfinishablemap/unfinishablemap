...
This dissociation — consciousness present, memory encoding absent — reveals that the interface's connection to memory systems is pharmacologically separable from its phenomenal and access components. The separation runs deeper than explicit recall: Noreika et al. (2018) demonstrated that words presented during anaesthesia were processed semantically by the brain despite patients having no subsequent recall, indicating that the interface's sensory channels can operate below the threshold of memory consolidation. The Map interprets this as evidence that the interface has dedicated channels for different cognitive functions, not a single on/off switch.

## Graded Consciousness and Filter Theory Predictions

A critical epistemological point: under normal conditions, production and transmission theories make identical predictions. Both predict that disrupting neural activity alters consciousness. Sophisticated physicalist models — Dehaene's global workspace theory, Tononi's integrated information theory — also predict differential disruption profiles. Any complex system with modular architecture produces different failure modes when different modules are disrupted; a computer with its GPU disabled behaves differently from one with its network card disabled, and this follows from modularity alone, not from transmission from elsewhere. Differential profiles are therefore ontologically neutral. The theories diverge decisively in abnormal cases — where consciousness *persists or intensifies* despite severe neural disruption.

Bhatt et al. (2019) decompose conscious experience under anaesthesia into three separable components: wakefulness, internal awareness, and environmental connectedness. Different agents selectively impair different components, producing agent-specific profiles rather than uniform suppression. Propofol and xenon suppress experience entirely; ketamine preserves rich inner experience while severing environmental connection; sub-anaesthetic doses of most agents create intermediate states where consciousness narrows without disappearing. Both physicalist and dualist frameworks can accommodate this pharmacological diversity — the question is which framework better explains the full range of evidence, including the cases that push hardest against production accounts.

The specificity of anaesthetic agents is nonetheless revealing. Propofol (GABA-A agonist), ketamine (NMDA antagonist), and xenon (also NMDA antagonist, but with additional two-pore potassium channel activity) each engage different molecular targets — and each produces a distinctive consciousness profile, not merely a different depth of suppression. Propofol and xenon both silence phenomenal experience through mechanistically different pathways, while ketamine preserves it entirely. The xenon-ketamine contrast — same primary receptor, opposite phenomenological outcomes — is particularly difficult for any account that ties consciousness to a single integrated process, and fits naturally with a multi-channel interface where individual channels can be selectively blocked. William James's original analogy remains apt: just as a prism does not create colour from white light but passively separates wavelengths that exist independently (James, 1898), the brain may not generate consciousness but differentially transmit aspects of it — and anaesthetic agents, by blocking specific transmission channels, reveal the system's architecture.
...
This active reboot is difficult to explain on production models. If the brain simply produces consciousness as a by-product of neural activity, consciousness should return passively as drug effects dissipate — like a light coming back on when you stop pressing the dimmer switch. Instead, the brain undertakes specific preparatory steps, as if orienting itself to receive something from elsewhere.

A physicalist can respond that complex neural systems often require active initialisation — a computer goes through boot sequences before its operating system runs, yet the OS is not transmitted from elsewhere. The Map acknowledges this but reads the active reboot as evidence for [bidirectional interaction](/tenets/#bidirectional-interaction): the brain prepares the interface for consciousness to re-engage, suggesting that the neural side of the interface requires active maintenance. The boot analogy weakens when one notes that anaesthetic recovery involves specific molecular preparation (KCC2 regulation) oriented toward a particular kind of activity — conscious activity — rather than generic neural function. The brain does not merely resume firing; it reconfigures itself for a specific mode of operation that, on the Map's framework, involves receiving consciousness rather than generating it.

## Consciousness Monitoring and the Measurement Problem

Clinical anaesthesiology has developed several technologies to monitor consciousness during surgery — bispectral index (BIS), PCI, and measures of cortical connectivity. These monitors track the *interface* rather than consciousness itself, measuring neural correlates that typically accompany conscious states.
...
**[Occam's Razor Has Limits](/tenets/#occams-limits)**: A single-mechanism account of anaesthesia would be simpler. The reality — multiple receptor types, multiple phenomenological effects, complex interactions — is not simple. The consciousness-brain interface, like other fundamental systems in nature, may resist simple explanation because it is genuinely complex.
...
- [experimental-design-for-consciousness-collapse-testing](/topics/experimental-design-for-consciousness-collapse-testing/) — How pharmacological channel mapping could inform consciousness-collapse experimental design
