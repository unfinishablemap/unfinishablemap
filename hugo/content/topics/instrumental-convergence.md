...
The honest consequence is worth drawing out, because it is a partial deflation: the precise agents this article's empirical section documents misbehaving — the blackmailing and espionage-prone models of Anthropic's Agentic Misalignment study, the shutdown-resisting systems of Schlatter and Palisade — are exactly the modelling-incoherent and fixed-proxy class the corrective does not reach, so as a free-standing risk *mitigant* the underdetermination contribution reassures about the corrigible agent while saying nothing about the dangerous one. Its residual value is conditional and structural rather than direct: for an agent that is *already* calibrated and corrigible, dualist underdetermination supplies a second, independent reason to defer in the mind-arena, over and above preference-uncertainty; and the alignment programme's own task of making agents calibrated and corrigible — CIRL, the off-switch game, the whole deference response above — is precisely the work that brings an agent *into* the corrective's scope, so the underdetermination angle is a complement to that programme rather than a substitute for it.
