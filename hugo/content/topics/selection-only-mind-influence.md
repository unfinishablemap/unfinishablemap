...
That is the whole of the rate-level arithmetic the strict reading currently supports. Born-rule preservation does not add a second, tighter formula, because it does not bear on throughput at all. What it binds is the *marginal* distribution over outcomes. [born-preserving-causal-efficacy](/apex/born-preserving-causal-efficacy/) states the relation exactly: on the averaging identity q(O | X) = Σ_C P(O | C, X) · P(C | X), Born-preservation constrains the left-hand marginal and says nothing about whether the conditionals on the right depart from it. Two scope conditions travel with that result. The first is the quantifier: preservation has to mean q(O | X) = q_Born(O | X) for *every publicly conditionable context X*, not merely for the pooled histogram, because a mind-state that a third party could label and condition on would carve sub-ensembles whose local frequencies depart from Born — and that is signalling. The second is that Born-preservation is not the only constraint bearing on the channel. What preservation alone secures is invisibility to *unconditioned* aggregate frequency tests, and it leaves the conditionals within a context free of any restriction *it* imposes; that is weaker than an open channel, because relativistic causality constrains the same channel independently, as *No-Signalling, Energy Conservation, and the Information-Side Constraint* below sets out. [P-Q7](/positions/quantum-interface/#p-q7) grades the no-signalling standing of that freedom as framework-internal only, with external evidence at grade C, so an unrestricted reading would outrun the Map's own register even without the causality constraint. [agency-budget](/concepts/agency-budget/) prices the alternative: Valentini's signal-locality theorem — scoped to deterministic hidden-variables theories and hypothetical non-equilibrium ensembles — makes a minimum *outside* the corridor statistically costly rather than free.
