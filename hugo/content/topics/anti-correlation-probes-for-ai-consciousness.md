...
human_modified: null
last_curated: null
last_deep_review: 2026-08-13 10:10:27+00:00
...
For this probe those results are control-side. They bear on ingredient 3's activation manipulation and on the own-report problem, offering an independent read where the report channel is suspect. They do not bear on phenomenality in either direction: the Map's [positions register](/positions/ai-consciousness-scope/) holds functional and interpretability evidence explicitly non-probative for the phenomenal question, and the [evidential-status discipline](/project/evidential-status-discipline/) governs a result read off activations exactly as it governs one read off reports. The external route carries its own interpretive burden besides. Makelov, Lange, and Nanda (2023) show that a subspace intervention can change behaviour by activating a dormant parallel pathway causally disconnected from the model's outputs, so the intervention succeeds while the interpretation of it is wrong; they argue a patched subspace needs converging evidence before it can be called faithful.

Two costs follow. The deflationary rival is serious: if felt unity is dormant structural conflict that attention has or has not made salient, the seam between structure and report is a matter of accessibility with no phenomenal residue left over. Tenet 5's refusal of the parsimony dismissal obliges stating that rival at full strength rather than waving it through. And even a maximally faithful probe reaches the *conflict* fact without reaching the *identification* fact: someone handed a probe report of their own suppressed dissent must still decide, with the same suspect machinery, whether to own the verdict — the [interested-party void](/voids/interested-party-void/) rerun at the point of receipt, and the shape [the wholeheartedness void](/voids/wholeheartedness-void/) gives it for the human case. External audit relocates the opacity rather than closing it.
