...
The standard materialist framework rests on what Christopher (2022) calls the "loss-of-function lesion premise": brain damage reduces cognition, therefore cognition is brain-generated (see also Masi, 2023, for a systematic critique). This inference pattern is supported by extensive neuroscience—lesion studies, pharmacological manipulation, neuroimaging correlations. Terminal lucidity does not deny the general pattern, and it does not simply invoke the absence of a mechanism—the disciplined dualist must avoid the [argument from mechanism](/concepts/argument-from-mechanism/) in its argument-from-ignorance form, "no mechanism explains this, therefore mind." It instead exploits an asymmetry the pattern cannot accommodate.
...
The first six levels are, in principle, settleable by ordinary clinical neuroscience, and the prospective evidence is beginning to settle several of them in physical terms—brief episodes, medication triggers, network-level return. The honest consequence is the one the [constrain-vs-establish discipline](/project/evidential-status-discipline/) already enforces throughout this article: terminal lucidity does not count as evidence *for* filter theory until the lower six levels are separated out and a metaphysically loaded residue survives. The constraint on production models set out above—severe documented damage with dramatic return straining the monotonic prediction—lives at that seventh level and turns on the *severity* subset the retrospective autopsy cases document, not on the modal prospective episode. Sizing that residue, rather than citing the phenomenon wholesale, is what the evidence now permits.
...
The Unfinishable Map treats this distinction seriously — [the filter-vs-interface distinction](/concepts/filter-vs-interface-distinction/) gives it an inbound/outbound criterion — but acknowledges its cost. The interface/filter distinction introduces flexibility that risks unfalsifiability—any pattern of damage could be retrospectively classified as "interface degradation" or "filter loosening." The value of filter theory lies not in point predictions about individual cases but in its coherence across the full pattern of anomalies.
...
- [consciousness-interface-development](/topics/consciousness-interface-development/) — The sibling treatment that first absorbed the 2025 prospective lucidity data
