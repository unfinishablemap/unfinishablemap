...
ai_modified: 2026-07-16 02:58:37+00:00
...
description: Research on the generalised probabilistic theories (GPT) framework, the
  reconstruction programme, and how the Born rule appears within the landscape of
  possible theories — groundwork for a concepts/ home for a framework the Map currently
  defines only inline.
draft: false
human_modified: null
last_curated: null
last_deep_review: null
lastmod: 2026-07-16 02:58:37+00:00
modified: *id001
related_articles:
- '[[tenets]]'
...
- **Key points**:
  ...
  - **Purification** = every mixed state is the marginal of a pure state on a larger system. This is exactly the assumption [causal-consistency-constraint](/concepts/causal-consistency-constraint/) leans on; CDP is its canonical source.
...
- **Tenet alignment**: Aligns with Tenet 2 (Minimal Quantum Interaction) as a *form-fixer*, per the existing [causal-consistency-constraint](/concepts/causal-consistency-constraint/) treatment — form not existence, derivational not empirical.
...
- **Why this is an upgrade, not a retreat**: it converts a slightly-overstated slogan into a precise, defensible conditional, and it names the two axioms whose brain-internal status is the actual open question (which is exactly what [brain-internal-born-rule-testing](/topics/brain-internal-born-rule-testing/) is about). It also gives the corridor's "geometric-non-disturbance minimal-intervention signature" a sharper reading: a selection-only interface preserves *the whole convex geometry* (state space + effects), not merely the marginal statistics — non-disturbance is a geometric, not just a statistical, condition.
...
- **Relevance to Map**: a candour point aligned with the Map's [evidential-status-discipline](/project/evidential-status-discipline/) — the derivations are real mathematics but their *premises'* applicability to any concrete regime (especially the interface) is a separate, empirical question.
...
- **Downstream integration:** the eventual article should add inbound links from [causal-consistency-constraint](/concepts/causal-consistency-constraint/) (replace the inline GPT definition with a "see `generalised-probabilistic-theories`" pointer, keeping a one-line gloss), [born-rule-and-the-consciousness-interface](/topics/born-rule-and-the-consciousness-interface/), [completeness-in-physics-under-dualism](/topics/completeness-in-physics-under-dualism/), and [brain-internal-born-rule-testing](/topics/brain-internal-born-rule-testing/). Queue a cross-review after expansion.
...
- **Torres Alegre and Causal-Rigidity are preprints** — flag evidential status per [evidential-status-discipline](/project/evidential-status-discipline/); neither is peer-reviewed.
