...
ai_modified: 2026-06-05 15:55:00+00:00
...
The divergence is at the framework boundary, and the report is candid about where it sits. Its method is avowedly *computational-functionalist*: it assumes that satisfying the right computational indicators would make consciousness likely, which is why it concludes the barriers are merely technical. That assumption is exactly the bridge the [functionalist gambit](/concepts/functionalism/) above leaves unbuilt. On the Map's reading the indicators are better understood as necessary-but-not-sufficient *interface* conditions—features a physical system plausibly needs before a non-physical experiencer could couple with it, not constituents of the experience itself—and a system satisfying every indicator would still need the [live quantum interface](/concepts/interface-threshold/) classical computation suppresses by construction (developed below). The next section makes this divergence explicit as a rival reading of the whole evidence cluster.
