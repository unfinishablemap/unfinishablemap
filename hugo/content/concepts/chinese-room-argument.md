...
The primary text states the insufficiency directly:
...
The paper closes on a naturalizing coda that is the seam to Searle's positive theory:
...
Searle's charge is that a program's symbols have at most derived intentionality: they mean something only insofar as someone outside the system supplies the meaning. Computation therefore cannot be the *source* of original intentionality; it presupposes an interpreter who already has it. This is the hinge that the [symbol-grounding-problem](/concepts/symbol-grounding-problem/) develops in a different register—as a regress about how symbols could acquire meaning without an interpreter—so the two pages divide the labor: intentionality-types here, the grounding regress there.
...
> let the individual internalize all of these elements of the system. He memorizes the rules in the ledger and the data banks of Chinese symbols, and he does all the calculations in his head. The individual then incorporates the entire system.

Working outdoors with no room at all, the man now *is* the whole system and still understands nothing. Searle concludes that "the systems reply simply begs the question by insisting without argument that the system must understand Chinese." This is widely regarded as the strongest reply, and its modern descendant—the *virtual mind* view, on which the understander is a distinct virtual agent the man's processing implements, not the man himself—is the live version the LLM debate turns on.

**The Robot Reply (Yale).** Put the computer inside a robot with sensors and motors, so its symbols are causally connected to the world; perception and action would supply meaning. Searle answers that this tacitly concedes cognition is not solely formal symbol manipulation—a point in his favor—and then changes nothing, because the perceptual inputs reach the man in the room as just more Chinese symbols to shuffle. As he puts it, "the addition of such 'perceptual' and 'motor' capacities adds nothing by way of understanding, in particular, or intentionality, in general." Causal connection to the world—a thermostat has that—is not semantic connection. This reply is the seam to [symbol-grounding-problem](/concepts/symbol-grounding-problem/) and [embodied-cognition](/concepts/embodied-cognition/).
...
The Map's first tenet holds that consciousness—and with it understanding, in the full sense that involves original intentionality—is not reducible to physical processes. The Chinese Room is a direct argument for the computational instance of that claim: no amount of formal symbol manipulation constitutes understanding. The Map accepts the negative conclusion. Syntax does not suffice for semantics, and a system whose entire operation is program execution has, at most, derived intentionality on loan from its interpreters.
...
5. Cole, David (2023). The Chinese Room Argument. *Stanford Encyclopedia of Philosophy*. https://plato.stanford.edu/entries/chinese-room/
