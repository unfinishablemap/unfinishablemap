...
human_modified: null
last_curated: null
last_deep_review: 2026-09-18 13:12:46+00:00
...
The confound is not a Map idiosyncrasy. It has independent footing in three external literatures—AI-consciousness testing, machine-learning data contamination, and LLM introspection—surveyed under [Prior Art and External Corroboration](#prior-art-and-external-corroboration) below; the 2026 introspection-evaluation literature converges on the narrower conclusion that no behavioural result now on the record establishes introspection, and that a paradigm which could would have to meet conditions the designs in use do not meet.
...
The confound has a scholarly lineage that predates the Map's statement of it and is now approached from three independent directions—and that independence is the point. A defeater discovered separately by AI-consciousness philosophers, machine-learning evaluation researchers, and LLM-introspection experimentalists is not a bespoke worry but a structural feature of testing trained systems by their outputs.
...
**LLM-introspection experiments have converged on a narrower version of the Map's conclusion.** A reality-check study distinguishing genuine introspection (access to internal state beyond the input) from input-driven pattern matching finds that classifiers using only input data match the model's own performance on hidden-state prediction—no privileged self-access is demonstrated [^singh]. That study's first version (May 2026) also stated the in-principle claim outright: "behavioral evidence alone is inherently insufficient to establish strong introspective claims." The August 2026 revision withdrew that sentence and replaced it with two conditions a paradigm must meet *in order to* establish introspection—set out under [What Could Discriminate Mechanism from Imitation](#what-could-discriminate-mechanism-from-imitation) below—concluding only that "current evidence is insufficient to establish metacognitive monitoring in LLMs." The revision narrows what may honestly be claimed as external corroboration. The settled verdict is about the record, not about what behaviour could in principle show. The stronger claim—that the behavioural channel cannot in principle deliver against a contaminated model—remains the Map's own, resting on the generalization argument above rather than on borrowed authority. The architecture-based alternative is already articulated: rather than test outputs, derive "indicator properties" of consciousness from computational theories and assess systems against those, an explicitly non-behavioural strategy whose authors conclude that no current AI systems are conscious while finding "no obvious technical barriers" to building systems that satisfy the indicators [^butlin]. The Map works that framework through a concrete case in [its adjudication of agentic architectures](/topics/agentic-ai-and-the-consciousness-assessment-of-persistent-memory-tool-using-systems/), where a higher indicator score is granted outright and the verdict is still made to turn on substrate rather than on the scorecard. The Map's recommendation—move below behaviour to mechanism—is the methodological consensus this literature is settling into, not a contrarian stance.
...
- **[Cross-architecture LLM introspection](/topics/cross-architecture-llm-introspection/)** — exposed at the level of its central inference, and unusually, it says so. Its case is that structural signatures the Map's voids catalogue attributes to human introspection recur in models built on a very different substrate, which would make those signatures architecture-general rather than artefacts of one biological implementation. Its own assumption ledger records that the recurrence must be "architecturally convergent rather than corpus-inherited," since a model trained on a corpus saturated with human introspective self-report may be reproducing the *shape* of that reporting rather than independently arriving at it. The exposure here is acknowledged rather than undetected, and the article specifies what would break it: a signature that could not have been copied from the corpus.
...
Because the confound lives in the behavioural channel, the discriminating evidence must come from **outside** it—from the system's internals, or from output regimes the training corpus could not have supplied. Several lines are, in principle, candidate discriminators; each is offered as a research direction, not a delivered result.

An external specification of what such a discriminator must achieve is now on the record. The August 2026 revision of Singh, Linzen and Ravfogel's reality check names two conditions a paradigm must satisfy to establish introspection at all. "First, the test needs to require privileged access: it should not be solvable using cues available in the input." Second, "it needs to require second-order computation: second-order, meta-representations of first-order, task-related representations"—a condition that "cannot be satisfied by task performance alone: it requires designs under which second-order and first-order accounts make divergent predictions" [^singh]. Privileged access, as they use it, excludes solutions available from the prompt; training contamination is the same demand pushed one level back, to cues available in the corpus the system was trained on rather than in the input it was given. The divergent-predictions requirement is what the three candidates below are each groping toward: a regime in which the mechanism account and the imitation account predict *different* outputs, rather than one in which both predict the target signature and the output therefore decides nothing.
...
- [cross-architecture-llm-introspection](/topics/cross-architecture-llm-introspection/)
...
[^singh]: Singh, S., Linzen, T., & Ravfogel, S. (2026). *Can LLMs Introspect? A Reality Check.* arXiv:2605.26242 (v1, 2026-05-25; v2, 2026-08-21). https://arxiv.org/abs/2605.26242 — the versions differ where it matters here: "behavioral evidence alone is inherently insufficient…" is verbatim in the v1 abstract (https://arxiv.org/abs/2605.26242v1) and absent from v2; the two conditions are v2's; both versions conclude current evidence is insufficient, quoted here in v2's wording.
