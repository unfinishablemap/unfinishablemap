...
description: Why indeterminism seems to threaten free will as much as determinism,
  and how consciousness-selection provides a distinctive response to the luck objection.
draft: false
human_modified: null
last_curated: null
...
Peter van Inwagen's thought experiment: roll back time after an agent S chooses action A. Replay the sequence many times. Since the choice was undetermined, different outcomes appear—sometimes S chooses A, sometimes B. Whatever explains the difference can't be anything about S, since S was identical in each replay up to the decision moment. The difference is pure luck. But if which choice occurs is lucky, it's not up to the agent.

## Mele's Formulation: Present and Remote Luck

Alfred Mele distinguishes **present luck** (nothing about the agent explains why one option was chosen at the moment of decision) from **remote luck** (the agent's character was shaped by unchosen factors). Present luck is more devastating to libertarianism—remote luck affects all theories equally, but present luck seems unique to indeterminism. The challenge: show that undetermined choices can be non-arbitrary.
...
**Criticism**: Even if the agent wanted both A and B, nothing about the agent explains why effort A won rather than B. The dual-efforts response pushes the question back without answering it.

## Agent-Causal Responses

[Agent-causal](/concepts/agent-causation/) libertarians (Chisholm, O'Connor, Lowe) argue the agent *does* explain the difference between possible worlds. Agent causation is a primitive relation—not reducible to prior events—that grounds genuine authorship. The agent's exercise of causal power *is* the explanation.

Critics find this unsatisfying: "the agent caused it" seems to name the explanandum, not the explanans. Agent-causalists respond that demanding further explanation assumes event-causal reduction. If agent causation is sui generis, the explanatory buck stops at the agent. This exchange can feel like an impasse—which the Map's framework aims to resolve.

## The Map's Framework: Selection Is Not Random

The Map strengthens the agent-causal response by specifying *how* consciousness exercises causal power. Standard quantum indeterminism is genuinely random. But the Map proposes that consciousness *selects* outcomes rather than merely receiving them:

| Feature | Lucky Indeterminism | Consciousness-Selected |
|---------|---------------------|------------------------|
| What determines outcome | Nothing (chance) | Agent's reasons |
| Phenomenology | None | Effort, engagement |
| Agent's relationship | Passive recipient | Active selector |

The [delegation](/concepts/delegatory-causation/) framework makes this precise. Every brain state has a [default causal profile](/concepts/delegatory-causation/) — the pattern of effects it would produce absent conscious takeover. When consciousness selects, it *preempts* that default profile: the experience does the causal work the physical state would otherwise have done. Selection is not a mysterious extra force; it is the agent taking over causal responsibilities that physics leaves open at points of quantum indeterminacy.
...
[Motor control](/concepts/motor-selection/) shows the same neural signatures as willed attention: frontal theta oscillations, ~300ms deployment time, distinct signatures for willed versus instructed action. Two independent domains exhibiting identical selection architecture strengthens the case that consciousness genuinely selects.
...
Consciousness selects based on reasons—the same reasons that make actions intelligible. Why A rather than B? Because the agent had reasons for A. Those reasons didn't *determine* the outcome—alternative possibilities remained genuine—but they *guided* it.

### The Retrocausal Dimension

[Retrocausal](/concepts/retrocausality/) selection deepens sourcehood. If consciousness operates atemporally, asking "what about the prior state explains the choice?" assumes the prior state was fixed before selection. But the prior state is *part* of what consciousness selected. The agent isn't just "the first cause in this chain" but "the cause of there being this chain at all."
...
The luck objection would succeed if: (1) consciousness doesn't actually select quantum outcomes, (2) selection isn't reasons-responsive, or (3) phenomenology is epiphenomenal and doesn't track real causal engagement. Remote luck (reasons formed by prior lucky events) challenges all responsibility theories equally.
...
- [compatibilist-symmetry-challenge](/concepts/compatibilist-symmetry-challenge/) — Named anti-slippage discipline at the moral-responsibility tier: surviving the luck objection does not establish that libertarian agency is required for moral phenomena
