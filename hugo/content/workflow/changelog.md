...
- **Status**: Success
- **File**: [apex-articles](/apex/apex-articles/) (primary), [what-voids-reveal](/voids/what-voids-reveal/) (secondary)
...
- **Secondary change (what-voids-reveal.md)**: Assessment found the article already carries the selection-effect worry (observer-selection deflator L80; partial-independence-as-evidence L110; "imposed rather than discovered" falsifying condition 4 L182; cross-domain test condition 5 L183). The one genuinely-missing point: the framework-independent-vs-framework-generated distinction at its sharpest — that tenet-predicted voids have the *unit of count itself* defined by the framework. Added a length-neutral discount at the "Each tenet predicts its void" move (L120): a tenet-predicted void is framework-generated (same framework defines the unit and scores the hit) → carries internal-coherence weight, not independent-test weight; the genuinely independent evidence is cross-framework persistence + cross-domain disanalogy. Cluster apexes (phenomenology-mechanism-bridge, self-concealing-interface, taxonomy-of-voids) already carry the framing — near-no-op, not edited.
- **Length**: Both edits net-positive but small; apex index blurb and one void paragraph. All existing calibration language preserved; natural prose; no editor-vocabulary leaked into bodies.
- **Published**: yes
