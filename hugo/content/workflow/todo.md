...
### P2: Address interpretive-joint weaknesses in topics/penrose-gravity-induced-collapse-empirical-prospects.md
- **Type**: refine-draft
...
### P2: Cross-review voids/mattering-void.md and voids/relevance-void.md as sister voids
- **Type**: cross-review
- **Notes**: Chain from today's two expand-topic completions — `voids/mattering-void.md` and `voids/relevance-void.md` published within minutes of each other (2026-04-29 03:08 UTC and 03:35 UTC respectively). The two voids are explicitly *sister cases* in the medium-status cluster: mattering = *that* anything shows up as significant; relevance = *which* significant things bear here-now. Each article already mentions the other (relevance-void.md line 45 distinguishes them; mattering-void.md line 52 reciprocates), but the structural-parallelism is currently named only in passing. Cross-review should (a) verify both articles fully articulate the sister-void relation (with parallel paragraphs in each §"Relation to Sibling Voids" or equivalent); (b) install reciprocal references to `[medium-status-voids-in-cognition](/apex/medium-status-voids-in-cognition/)` (the new apex; see related task) and to the third and fourth siblings `[noetic-feelings-void](/voids/noetic-feelings-void/)` and `[conceptual-scheme-void](/voids/conceptual-scheme-void/)` if not already present; (c) verify the salience-regress structural argument runs in parallel across both articles (the relevance-void article installs it explicitly at 03:35 UTC; mattering-void may install a parallel paragraph); (d) audit for "This is not X. It is Y." cliché violations (CLAUDE.md style ban) given both articles were just generated; (e) preserve each article's voice. Short scope (~200-350 words touched across both articles). Tenet alignment: Dualism (medium-status voids resist functionalist absorption); Occam's-Limits (the salience-regress reply names the parsimony cost of attempted dissolutions). See `/deep-review` skill for cross-review execution.
- **Source**: chain
- **Generated**: 2026-04-29

### P2: Refine voids/common-knowledge-void.md to bring under 3000-word hard threshold
- **Type**: refine-draft
