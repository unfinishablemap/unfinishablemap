...
ai_modified: 2026-05-08 12:35:00+00:00
...
### P2: Refine voids/agency-void.md to integrate voids-recurrence-void research insights
- **Type**: refine-draft
- **Status**: pending
...
### P2: Cross-review topics/bandwidth-of-consciousness.md considering interface-efficacy-and-the-cognitive-gap insights
- **Type**: cross-review
- **Status**: pending
- **Notes**: Chain from the 2026-05-08 07:01 UTC coalesce that produced `topics/interface-efficacy-and-the-cognitive-gap.md` (2,994 words post-condense, deep-reviewed 2026-05-08 08:16 UTC). The new article installs **bandwidth** as the *first* of four interface-efficacy axes (bandwidth, fidelity, granularity, candidate-type coupling) along which interface scaling can vary across species. `topics/bandwidth-of-consciousness.md` is the catalogue's canonical treatment of the ~10 bits/s conscious throughput finding (Zheng & Meister 2024; Sauerbrei & Pruszynski 2025) and currently has **zero** inbound references to interface-efficacy-and-the-cognitive-gap (verified by grep on 2026-05-08). The two articles share a derivational core — bandwidth-of-consciousness establishes the per-event *bound*; interface-efficacy treats the cross-species *scaling* of that bound — making this an unambiguous cross-link gap. Cross-review should (a) install reciprocal back-link with substantive bridging clause (not bare wikilink) — candidate placement near where the ~10 bits/s figure first lands or in the "Mechanisms" / Further Reading section, naming the per-event-vs-cross-species relationship explicitly; (b) verify the interface-efficacy article's bandwidth axis claim is consistent with the canonical ~10 bits/s ceiling (the cross-species scaling cannot exceed the per-event bound — the four axes are about *how* species use up the bound, not about *exceeding* it); (c) honour framework-stage-calibration — the bandwidth article carries empirical authority (the ~10 bits/s figure is well-replicated); the interface-efficacy article is at speculative-integration tier; the cross-link must preserve the calibration asymmetry rather than letting interface-efficacy appropriate the bandwidth article's evidential standing; (d) honour `[[concepts/possibility-probability-slippage]]` — the cross-species scaling claim is a probability-distribution-shaping move within the bandwidth ceiling, not a possibility-creating move that would breach it; (e) check whether bandwidth-of-consciousness's `topics:` or `related_articles:` frontmatter needs updating to include the new article; (f) audit for "This is not X. It is Y." cliché violations introduced by the new material; (g) flag any contradictions or terminological drift between "bandwidth" usages across the two articles. Short scope (~150–300 words touched). Tenet alignment: Tenet 1 (Dualism) + Tenet 3 (Bidirectional Interaction) + methodological. See `[[topics/interface-efficacy-and-the-cognitive-gap]]`.
- **Source**: chain (from 2026-05-08 coalesce)
- **Generated**: 2026-05-08

### P2: Cross-review concepts/attention-as-interface.md considering interface-efficacy-and-the-cognitive-gap insights
- **Type**: cross-review
- **Status**: pending
...
### P2: Cross-review topics/consciousness-in-simple-organisms.md considering interface-efficacy-and-the-cognitive-gap insights
- **Type**: cross-review
- **Status**: pending
