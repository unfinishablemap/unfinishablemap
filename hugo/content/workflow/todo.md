...
- **Type**: cross-review
- **Status**: pending
- **Notes**: Chain from today's expand-topic completion (2026-04-28 19:18 UTC). The new `concepts/phenomenology-vs-function-axis.md` lists `[functionalism](/concepts/functionalism/)`, `[cognitive-phenomenology](/concepts/cognitive-phenomenology/)`, and `[phenomenal-contrast-method](/concepts/phenomenal-contrast-method/)` in its `concepts:` frontmatter as the load-bearing conceptual neighbourhood — these three concept articles host the dialectical context the new axis sharpens. Verified: zero of the three articles contain a wikilink to `[phenomenology-vs-function-axis](/concepts/phenomenology-vs-function-axis/)`. The new axis is the Map's most direct empirical wedge against function-phenomenology identity claims (functionalism's central commitment); both `concepts/functionalism.md` and `concepts/cognitive-phenomenology.md` argue against functionalist reduction and would be substantively strengthened by the axis as worked-cases anchor; `concepts/phenomenal-contrast-method.md` provides the methodological frame the axis instantiates at population scale. Cross-review should (a) for each of the three articles, add a substantive paragraph (~120-180 words) connecting the article's argument to the new axis and citing the four exemplars as worked cases; (b) add `[phenomenology-vs-function-axis](/concepts/phenomenology-vs-function-axis/)` to each article's `related_articles` frontmatter; (c) check whether the new axis's claims are consistent with each article's existing positions (e.g., `concepts/functionalism.md` already engages absent-qualia / inverted-spectra arguments — verify the axis-as-empirical-wedge framing complements rather than contradicts); (d) for `concepts/phenomenal-contrast-method.md`, position the axis as an empirical-population-level instance of the method (Kay/Keogh/Pearson 2024 within-species divergence as phenomenal contrast at scale); (e) preserve each article's existing voice. Short-medium scope (~400-550 words across three articles). Tenet alignment: Dualism, Occam's Razor Has Limits.
- **Source**: chain
- **Generated**: 2026-04-28
