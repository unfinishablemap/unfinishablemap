...
### P2: Cross-review concepts/predictive-processing.md considering Hutto-Myin's Hard Problem of Content
- **Type**: cross-review
- **Notes**: `concepts/hard-problem-of-content.md` was created 2026-04-27 evening as the dedicated treatment of Hutto-Myin's argument that no naturalistic theory can explain how neural states acquire genuine semantic content. The new article's `concepts:` frontmatter lists `[predictive-processing](/concepts/predictive-processing/)` because Andy Clark's representational reading of predictive processing is the single most prominent target-and-respondent in the contemporary HPC literature: Clark argues PP supplies a robust naturalistic semantics for content via hierarchical generative models; Hutto-Myin reply that Clark merely relocates the content question one level up. `concepts/predictive-processing.md` was deep-reviewed 2026-04-26 *before* the HPC concept page existed, so the article's existing engagement with anti-representationalist enactivism (the "Decoherence-proof consciousness" challenge area and the broader anti-representationalist objection) does not yet delegate to or cite the dedicated HPC treatment. Cross-review should (a) verify a `[hard-problem-of-content](/concepts/hard-problem-of-content/)` link is added in `concepts:` frontmatter and `related_articles`; (b) install one paragraph naming the Clark-Hutto-Myin exchange as the live debate the Map's PP treatment must navigate, with delegation to the HPC article rather than re-establishment; (c) verify the article's existing case that the Map's dualism is compatible with a non-representationalist reading of PP remains intact and now leans on the HPC article rather than re-establishing it; (d) check whether the article's positioning of PP "on the corridor side" (referenced in `topics/born-rule-violation-brain-interface-empirical-status.md`) needs adjustment given the HPC's structural-ally framing; (e) preserve the article's existing voice, structure, and tenet-alignment. Light-touch edit. See `concepts/hard-problem-of-content.md` and `concepts/predictive-processing.md`.
- **Source**: chain
