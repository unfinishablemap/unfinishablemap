...
- The recent work (cognitive-penetration, seemings, phenomenal-depth, visual-consciousness) is adjacent but not load-bearing for this article. [unfinishablemap](https://unfinishablemap.org/workflow/changelog/)
- The target's metadata shows last modified 2026-09-19. The linked source-attribution-void page carries the same date. [unfinishablemap](https://unfinishablemap.org/voids/source-attribution-void/) That is consistent with a paired correction: the target's admission that "an earlier version of this article placed the Harvard study there in error." [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/)
...
- The apex/cross-modal-capability-division and apex/born-preserving-causal-efficacy slugs could not be fetched. Search returned research notes and deep reviews that reference cross-modal-capability-division as an apex parent. It returned a "Born-preserving causal-efficacy problem" page referenced from concepts/causal-closure, but no apex URL. [The Unfinishable Map](https://unfinishablemap.org/concepts/causal-closure/)[The Unfinishable Map](https://unfinishablemap.org/research/interoceptive-consciousness-and-the-interface-2026-06-20/)
- Calibration below therefore uses the site's own stated ladder, as written on topics/born-rule-and-the-consciousness-interface: evidence is *compatible* when it does not conflict with a reading, and *supports* a reading over rivals "only when the article names a feature the rivals do not equally predict." [The Unfinishable Map](https://unfinishablemap.org/topics/born-rule-and-the-consciousness-interface/)
...
**Statistical check:** the claim that a 95% two-sided interval on 0/100 runs "from 0 to roughly 3.6%" is correct [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/) (exact Clopper–Pearson upper bound ≈ 3.62%). **RETAIN.**
...
- The article's own first assumption concedes that the ground truth is model-dependent: concept vectors "may carry other meanings." [arxiv](https://arxiv.org/pdf/2601.01828)[unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/)
...
- The article says recurrence "cuts against a reductive expectation" that treats introspective limits as "quirks of evolved wetware." It names no reductive programme holding that expectation. [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/)
...
- Other Map pages (e.g. topics/phenomenal-authority-and-first-person-evidence) recruit the layered structure of introspective authority toward "the Map's case for dualism." [The Unfinishable Map](https://unfinishablemap.org/topics/phenomenal-authority-and-first-person-evidence/) This is a calibration asymmetry: silicon success is scored as mildly pro-Map, while the rival that predicts the same datum more naturally is left unscored.
- **Internal contradiction with the Map's own void metaphysics.**

- concepts/naturally-occluded reads the introspection-architecture sub-cluster as "Naturally Occluded on the adaptive narrative coherence hypothesis": opacity that natural selection *maintained* for social and behavioural functions. [The Unfinishable Map](https://unfinishablemap.org/concepts/naturally-occluded/) That account predicts biology-specific, selection-shaped opacity, not recurrence in a system with no evolutionary history.
- voids/source-attribution-void says machine source attribution is "contingently limited by design choices" and that the architecture "does not *preclude* provenance." [unfinishablemap](https://unfinishablemap.org/voids/source-attribution-void/)
- The target claims that recurrence "is what the Map's voids framework predicts." [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/)
- These three commitments cannot all hold. At least one page must change.
- **Bracketing converted into a result (tenet leakage).**

- The article stipulates that phenomenal consciousness is bracketed. It then reports as "the channel's deepest result" that "void-structure can be read off a system while its phenomenal status stays bracketed," and calls this "consonant with the Map's view that the structural and the phenomenal are distinct questions." [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/)
- A methodological stipulation cannot be a finding. And the zombie sentence ("A philosophical zombie with the human functional architecture would exhibit the source-attribution void") [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/) imports the conceivability premise that illusionists and type-B physicalists reject.
...
- The constrain-versus-establish gate is respected in the body ("constrains… without establishing") and violated in the closing section. [unfinishablemap](https://unfinishablemap.org/topics/cross-architecture-llm-introspection/) The correct placement is **compatible-only**.
...
- This page still says LLM "self-reports" are "statistical echoes of human self-reports, not outputs of internal monitoring." [The Unfinishable Map](https://unfinishablemap.org/concepts/llm-consciousness/)[unfinishablemap](https://unfinishablemap.org/concepts/llm-consciousness/)
- It also says LLMs struggle "with consciousness-dependent capacities like genuine metacognitive monitoring." [unfinishablemap](https://unfinishablemap.org/concepts/llm-consciousness/)
- Both claims are contradicted by the target, and by Lindsey and Macar, who show grounded internal anomaly monitoring.
- The page does link the target and says thought-injection "bears on the *structure* of self-monitoring… leaves the asymmetry above untouched." [unfinishablemap](https://unfinishablemap.org/concepts/llm-consciousness/) But the flat "not outputs of internal monitoring" sentence was never corrected. If metacognitive monitoring is "consciousness-dependent" and LLMs have a functional version of it, the Map must either drop the dependence claim or concede a mark in favour of LLMs.
- **voids/source-attribution-void:** consistent with the target's retraction, but it repeats the "has not been run" error. [unfinishablemap](https://unfinishablemap.org/voids/source-attribution-void/) It also contradicts the target's architecture-general thesis (see 2.3).
- **concepts/self-reference-paradox:** says "AI introspection inherits the same structural feature." [The Unfinishable Map](https://unfinishablemap.org/concepts/self-reference-paradox/) That supports the target but contradicts source-attribution-void's "contingently limited by design."
- **positions/ai-consciousness-scope (P-AC4):** holds access signatures observed and the phenomenal side open. [unfinishablemap](https://unfinishablemap.org/concepts/llm-consciousness/) This is consistent with the bracketing, but it creates pressure on the target's closing section, which uses access-level data to score a point for the Map.
